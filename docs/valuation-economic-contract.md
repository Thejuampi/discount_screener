...
| ~~**LD-12**~~ | ~~**No test in this repository fails when a published intrinsic value changes.** The observation audit dumps a cohort table and asserts nothing about the values in it; the parity suite asserts `path.is_file()` and `members.len() == 20`. A green suite is therefore not evidence that published values held, and every wave in this run that claimed "nothing published moved" proved it by pair-measuring two builds by hand, not by a gate.~~ | **CLOSED BY WAVE R5.** `valuation_core_measurement::published_value_regression_gate` (not `#[ignore]`, runs in default `cargo test --lib`) reads `tests/fixtures/valuation/published_value_regression_gate_cohort.json` and fails, naming the specific issuer, if either engine's published cents value or refusal reason moves for any of the 20 pinned cohort members. Refusal reason is pinned as a first-class golden value, not a placeholder. Re-blessing requires a person to run the separate `#[ignore]`d `bless_published_value_regression_gate_cohort` writer explicitly — a normal test run never rewrites the fixture. | — | `published_value_regression_gate` fails the moment any pinned issuer's legacy value, Core value, or Core refusal reason diverges from the golden fixture, and names the issuer in the assertion message. |
| ~~**LD-13**~~ | ~~`valuation_fixture_capture.rs:138,140` fabricates two fields it has no evidence for — `"effective_tax_bps": point.tax_rate_bps.unwrap_or(0)` and `"marginal_tax_bps": point.marginal_tax_bps.unwrap_or(2_100)` — three lines below a comment reading *"Filling any of them would be inventing the very history this capture exists to stop inventing."* A committed fixture therefore carries a 21% statutory guess and a 0% floor as though both were filed facts. Latent second half: `valuation_baseline.rs:72` declares `interest: f64`, while the emitter can now write `null` for it.~~ | **CLOSED BY WAVE R6, for the emitter and the types only — it does not close for the corpus (see LD-17).** The emitter now writes `point.tax_rate_bps` / `point.marginal_tax_bps` verbatim (`null` when absent) with no `unwrap_or`. `DriverAnnual::effective_tax_bps` / `::marginal_tax_bps` (`valuation_baseline.rs`) are `Option<i32>`; `IssuerAnnual` construction in `valuation_core_measurement.rs::evidence` drops a year rather than default its marginal tax rate when absent. The committed `core_driver_data_deep.json` is untouched and still carries zero nulls, so no published value moved. | — | `deep_driver_year_row_never_fabricates_effective_tax_rate` and `deep_driver_year_row_never_fabricates_marginal_tax_rate` (`valuation_fixture_capture.rs`) fail if the emitter ever re-introduces a default; `DriverAnnual`'s fields returning to non-`Option` would fail to compile every honest consumer again. |
...
| **LD-17** | The committed `tests/fixtures/valuation/core_driver_data_deep.json` predates LD-13's closure and carries **zero nulls** in `effective_tax_bps` / `marginal_tax_bps`, so the fabrication LD-13 stopped the *emitter* from repeating is still baked into the *corpus* it already wrote: 179 of 274 issuer-years read `marginal_tax_bps == 2100` and 24 read `effective_tax_bps == 0`, and nothing in the file distinguishes a genuinely filed value from the old `unwrap_or` guess at either site. | Correcting the corpus needs a network re-capture (`capture_the_deep_driver_fixture`, `#[ignore]`d, SEC EDGAR), which would move the deep-driver evidence `published_value_regression_gate` reads (via `cohort_evidence`/`evidence` in `valuation_core_measurement.rs`) and is a separate gated event, not a type-and-emitter change. | Any wave that re-captures `core_driver_data_deep.json`, or any claim that the Core-side cohort's tax evidence is audited. | **Mechanical, one-time**: after a re-capture, `published_value_regression_gate` must stay green — the gate does read this fixture today, but every pinned issuer's Core outcome is `Refused{estimator_unavailable\|not_reported}` upstream of `free_cash_flow`'s marginal-tax read (LD-12's pinned golden), so the corpus's fabricated rates are not load-bearing for any *currently* pinned value. The moment a Core estimator is promoted and starts publishing off this fixture, `annual.marginal_tax_bps` (`valuation_core_adapter.rs:517`) becomes load-bearing and a re-capture must be pair-measured against the gate, not just re-blessed. **None for the corpus half itself** — counting nulls in the file is the only audit, and today there are none to count. |
