...
This architecture defines source-resolved inputs, provider states, deterministic ties, disagreement tests, fiscal alignment, concurrency, cache fingerprints, and promotion gates.
...
Before SEC-primary is enabled, add a dated evidence file under `docs/research/provider-samples/`. Record five upstream samples and all release-gate results.
