...
- [Live QA, Windows SEC sieve, 2026-08-29](../_bmad-output/implementation-artifacts/live-qa-windows-sec-sieve-2026-08-29.md) - the streaming companyfacts sieve and the shared per-issuer cache running in the desktop app. AAPL and MSFT both returned an FCFF value, which no sieved document can produce if the sieve drops a driver. The bank path did not run: the `qa` profile holds no financial-class name.
