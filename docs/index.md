# Discount Screener Documentation

Use this page to find the current source for each concern.

## Product

- [Current functionality](product/current-functionality.md)
- [Advisor CSV import](product/advisor-csv-import.md)
- [Android Positions](product/android-positions.md)
- [Android Earnings gate](product/earnings-gate.md)
- [Android Dip hunt](product/android-plans-dip.md)
- [Android Cross hunt](product/android-plans-cross.md)
- [Android Leftover hunt](product/android-plans-leftover.md)
- [Deferred work](product/deferred-work.md)

## Architecture

- [Valuation model family](architecture/valuation-model-family.md)
- [DCF source consistency](architecture/dcf-source-consistency.md)
- [Cross-platform parity](architecture/cross-platform-parity.md)
- [Shared contracts](../shared/contracts/README.md)
- [PUML runtime](architecture/puml-runtime.md)
- [Scoring evaluation snapshots](architecture/scoring-evaluation-snapshots.md)

## Operations

- [Valuation live QA](operations/valuation-live-qa.md)
- [Valuation baseline policy](operations/valuation-baseline-policy.md)
- [Operational anti-patterns](operations/operational-anti-patterns.md)
- [Analyst-method lifecycle](operations/analyst-method-lifecycle.md)
- [Windows UI inspection](operations/windows-ui-inspect.md)
- [Windows dashboard regression](operations/windows-dashboard-regression.md)

## Application Guides

- [Repository overview](../README.md)
- [Desktop](../apps/desktop/README.md)
- [Windows](../apps/windows/README.md)
- [Android](../apps/android/README.md)
- [Flutter](../apps/flutter/README.md)

## Research And Diagnostics

- [Research](research/)
- [Yahoo API loading research](research/yahoo-api-loading-2026-09-25.md)
- [Windows loading optimization](research/windows-loading-optimization-2026-09-26.md)
- [Aggressive V5 evaluation](research/aggressive-v5-evaluation.md)
- [Scoring evaluation process design](research/scoring-evaluation-process-design.md)
- [Scoring evaluation validation](research/scoring-evaluation-validation.md)
- [Diagnostics](diagnostics/)
- [Quant references](references/quant/)
- [Lab evidence](../lab/)

## History

- [Documentation archive](archive/README.md)
- [Unmerged change review, 2026-09-26](archive/unmerged-change-review-2026-09-26.md)

Archived files do not define current behavior.

## Maintenance

- Keep one current home for each fact.
- Use links instead of copied rules.
- Add durable documents to this index.
- Move obsolete plans to `archive`.
