...
- **Honesty modes.** Working identity is `ValuationHonesty.Honest`. Street-implied one-knob inversions are a parallel `NonHonest` signal (`StreetImpliedView`, policy `street-implied-honesty/3`). Publish implied bps, delta vs honest, and stretch (`Modest` / `Stretched` / `Absurd` / `Unreachable`). `ape_nh` near 0 is inversion, not a win. Do not mix Street into honest inputs or into `price-forecast`. Detail labels the winning stretch. Exhaust honest path work before using non-honest freedom.
