...
- **Honesty modes.** Working identity is `ValuationHonesty.Honest`. Street-implied one-knob inversions are a parallel `NonHonest` signal (`StreetImpliedView`, policy `street-implied-honesty/3`). Compute both on one run. Detail prints both dollars. The non-honest line names the bent input and the two rates. Publish implied bps, delta vs honest, and stretch (`Modest` / `Stretched` / `Absurd` / `Unreachable`). `ape_nh` near 0 is inversion, not a win. Do not mix Street into honest inputs or into `price-forecast`. Do not switch the working number to NonHonest. Exhaust honest path work before using non-honest freedom.
