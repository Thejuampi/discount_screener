# `plan.v4.md` — Valuation PIT & Contract (E2E session `valuation-pit-contract`)

Branch: `valuation/wave1-integration` · Repo: `G:\dev\repos\discount_screener`
Supersedes `plan.v3.md` in full. This is a replacement plan, not a patch.
...
**Ruling, split by class because the two are not the same defect.** COF: accept the refusal —
Decision 1 applies, and applies *cheaply*, because no coverage is lost to weigh against it.
DAL/CHTR/BKR: ship the sign fix and carry the net-vs-gross residual as **LD-8**, which already
states honestly that it has no mechanical detector. Refusing all three would discard *real, filed,
measured* evidence; Decision 1 says coverage is not a **promotion gate**, not that supported
evidence should be thrown away. This project already ran the alternative — the credit-curve episode
went 15-of-20 dark (`prd-discount_screener-2026-08-03/.memlog.md:26-29`) and was resolved by fixing
five real evidence bugs and *keeping* real evidence, never by fabricating a rung.
...
- **J10** *(new in v3; renumbered from a colliding second `J7` — the id was already taken by the
  ...
  *performed* the abs; **J7 is the search for the sites that depended on it.** Enumerate with
...
*One thing the builder must check first, and report if it fails:* that the existing heading shape
can carry the **D7 latent-defect register** cleanly — it is now **eleven** rows (LD-1 … LD-11), not
seven. If it cannot, the register moves to `docs/valuation-economic-contract.md` and `AD-VM-012`
links to it; the register does not get silently truncated to fit a heading.
...
- **the latent-defect register (D7), `LD-1` through `LD-11` enumerated by id** — LD-1, LD-2, LD-3,
  LD-4, LD-5, LD-6, LD-7, LD-8, LD-9, **LD-10**, **LD-11** — each with its owner, trigger condition
  and detector. *(v3 wrote "LD-1 through LD-7" while the register already ran to LD-9; a range that
  silently excludes the newest entries lets a wave be declared done without them. Ranges are
  enumerated by id throughout v4 — Sensei r4.)*
...
## 10. Changelog, v0 to v1 (retained for the record)
