# `plan.v5.md` — Valuation PIT & Contract (E2E session `valuation-pit-contract`)

Branch: `valuation/wave1-integration` · Repo: `G:\dev\repos\discount_screener`
Supersedes `plan.v4.md` in full. This is a replacement plan, not a patch.

Inputs, in precedence order: **Juan's scope ruling of 2026-08-04 — Android is out of scope, Windows
only (§0.0)** → **Juan's rulings on Q1 and Q2 (2026-08-04, §0)** → `brief.md` (Juan's decisions 1–3,
13 binding constraints) → **`plan-review/ORCHESTRATOR-RULINGS.md` (R-1 … R-6, the orchestrator's own
decisions with their evidence)** → `plan-review/r1-consolidated-directives.md`, the r2 round, the r3
round, and the **r4 round (Sensei `revise`, 7 P0 / 5 passes; Advisor `revise`, 2 P0 / 4 passes)** →
`refine.md` → `plan.v3.md` (preserved where unfaulted).

## §0.0 — Scope: Windows only

Juan, 2026-08-04, mid-session and verbatim:

> "android esta fuera de scope ahora. el scope es windows. cuando consigamos una solucion buena en
> windows, hacemos el port a android."

This is a reaffirmation, not a new decision — `brief.md:214` already listed **"Android parity"** in
`EXPLICITLY OUT OF SCOPE`, alongside AMZN policy/16, the ROIC fixture and re-capturing
`core_driver_data_deep.json`.

**What it does and does not cut** — the full reasoning, with the three compile-safety checks that
back it, is `ORCHESTRATOR-RULINGS.md` R-3:

| | |
|---|---|
| **stays** | the contract gains `qnameSigns`; fingerprint `/8` → `/9`; the generator emits **both** targets; generated Kotlin gains the field *inertly* |
| **cut** | T2.5 (hand-written Kotlin negation + its Kotlin test), T2.11, W2-P04, W2-P05, the Gradle run |

Emitting to Rust only would deliberately desynchronize the generator from its own contract —
manufacturing the exact drift §1 exists to repair. **Scope reduction is not a licence to introduce
drift.** The generated Kotlin carrying the field costs nothing and compiles: `GeneratedSecDriverOperator`
is declared inside the generated file (`SecDriverNormalizationPolicyGenerated.kt:4`), no other Kotlin
file references it, its one consumer reads it by *named field* (`SecDriverNormalizationPolicy.kt:72-81`),
and nothing destructures or `copy()`s it — the only two constructs an added field could break.
*[executed 2026-08-04]*

**The cost, stated once and not buried:** from W2b onward Rust keeps the interest sign and Kotlin
still `abs()`es it (`DcfAnalysisEngine.kt:535-541`, `:802`). The platforms genuinely diverge, by
choice. It is registered as **LD-10** with a mechanical trigger and detector, not a comment.

## How to read a verification claim in this document

**Second clause, added in v3 and binding.** The rule below governs the plan's *claims*. It did not
prevent a fourth instance of the same failure, because the fourth instance was in the plan's
*checks*: v2's fast-checks table shipped `grep -nE "interest.*(abs\(\)\|f64::abs)"`, which under
`-E` reads `\|` as a literal pipe and matches nothing — a builder running it would have seen a
clean result with all three `.abs()` sites intact. Therefore:

> **An acceptance check must be executed once, against a known-positive case, before it is written
> into this plan.** A check that has never been observed to fail when it should is not a check. This
> applies to every grep, every test invocation and every gate command in every wave.

The tool is **PowerShell**, not `grep` — this repo is PowerShell-primary.

**Retraction, and the third instance of this document's own failure mode.** v3 first wrote here:
*"Every acceptance check in this document names the tool it runs under and has been executed against
a case known to match."* **That sentence was false when written.** It is a blanket claim over roughly
forty checks across five waves, of which the orchestrator executed only the ones it added; the rest
are inherited from v1 and v2 and were never run. On its first use, the executed-check rule produced
an unexecuted summary claim about execution. It is struck, and replaced by a per-check ledger:

> Every acceptance check carries **`[executed <date>]`** or **`[inherited — not executed]`**. One
> token per check. An unexecuted check that is labelled unexecuted is honest; a blanket claim
> converts forty unknowns into forty assertions.

**Third clause — this one binds the builder, not the plan author.** The plan cannot pre-execute a
check whose known-positive case does not exist yet: T5.4's `reason` cells, T2.6's three new tests
and W3-P04's counters are all written *during* their wave. So:

> **A check must be observed to fail before it is relied on.** For a search: run it against the
> pre-change tree and record the rows it returns. For a test: run it against unmodified code and
> record the failure. **Evidence of pass carries both outputs** — the known-positive and the final
> clean one. A single clean output is indistinguishable from a check that cannot fire.

That is red-then-green restated as a verification rule, and it is the only form that survives the
builder writing the check.

> **Fourth clause, added in v6 — red is not enough; the red must be *isolated*.** Wave 3 found one
> of its own pins passing under mutation: a whole-fit-refusal mutation returned `None` before
> `growth_pooled_discarded` was ever written, so the counter stayed `0` and
> `a_clean_cohort_discards_nothing` held **vacuously**. It only surfaced when the threshold mutation
> was isolated in a second round.
>
> **A combined mutation that turns everything red is exactly the evidence shape that hides a test
> which never fired.** Therefore: **every "observed failing" claim states whether the mutation was
> combined or isolated**, and any check whose subject can be short-circuited upstream — counters,
> diagnostics, anything written late in a function that can return early — **must be pinned by an
> isolated mutation**. A wave reporting only a combined round has recorded that everything went red,
> which is not the same as recording that each assertion is load-bearing. *(R-8.4)*
...
**LD-1 is a three-site removal, not one line** (verified with **the canonical check named in J6** (PowerShell `Select-String -Pattern 'interest.*(abs\(\)|f64::abs)'`); the naive `interest.*abs()` finds only two of the three, missing the setter at `:907` because `.map(f64::abs)` has no parentheses
plus the setter):
...
| F16 | `docs/` is flat, contains no ADR **file**, and `docs/index.md` has a "Maintenance Rules" section requiring it be kept current. `sec-driver-normalization.json` and its fixtures appear in **neither** `docs/index.md` **nor** `shared/contracts/README.md`'s `## Files` list. | verified |
| **F16b** | **v3's reading of F16 as "this repo has no ADR" was wrong, and it is only true *inside* `docs/`.** `_bmad-output/planning-artifacts/valuation-model-family-architecture.md` is a live numbered valuation decision log, **`AD-VM-001` … `AD-VM-011`**, and `AGENTS.md:590`'s Documentation Map already links it under the label **"valuation ADRs"**. `AD-VM-003` is literally *"Financial Services Use Residual Income / Excess Return on Equity"* — the same dispatch that R-5.0 verified in code. *(Advisor r4 P1-2.)* | **verified** *[executed 2026-08-04]* |
...
- ~~Therefore **fix `KotlinList`'s empty case anyway** (T2.11)~~ — **struck by §0.0.** Kotlin-output-only, Windows unaffected, and `qname_signs` cannot be empty because every driver has qnames. Deferred to the port. *(This is a binding D-decision that still ordered cut work — Sensei r5 P0-5.)* It was a latent generator defect
...
| **LD-8** | **T2.7's sign rule is one-sided, and the silent half is the larger one.** Refusing on `interest < 0.0` catches net-**income** filers. It never fires for a net-**expense** filer, whose `InterestIncomeExpenseNet` series is *equally net* — income has been subtracted from it — and is then fitted as though it were gross interest expense. The cost of debt is understated for that issuer, and the number looks entirely plausible. | The correct rule keys on the **basis** of the series (net vs gross), not on the sign of its value. That is unimplementable until `FcfPoint` carries per-field concept provenance, which is item 6 and out of scope. The sign rule is the right approximation available now, and T2.7 must say that it is an approximation rather than presenting it as the rule. | Item 6's provenance work; or the first estimator pre-registered against a cost-of-debt input. | **No mechanical detector, and that is the point** — this failure produces plausible numbers. Human review checkpoint. **The exposed set is three, not four: DAL, CHTR, BKR.** COF is a net-*income* filer but is classified `FinancialServices` and dispatched to `residual_income` at `dcf_model.rs:1263-1265` — `fcf_history` is not even passed — so it never reaches `fcff_wacc` → `derive_wacc` and cannot be exposed to this defect through a published number *(v3 listed four; corrected in v4, `ORCHESTRATOR-RULINGS.md` R-5.0)*. A fourth operating filer is undetectable today. |
| **LD-9** | `posterior::measured_sample_size` (`posterior.rs:119-125`) sums `basis().sample_size()` across channels **without regard to variant**, adding `SampleVariance{observations}` (periods) to `AnalystDispersion{analysts}` (people) to `Propagated{inputs}`. The fused posterior is then labelled with that sum. | Cosmetic today: nothing consumes `sample_size()` arithmetically — verified, `fuse` weights by `Observation::precision()` = `1.0/variance` only. It becomes load-bearing the moment any consumer reads the fused basis as an `n`. | The first consumer that reads a fused `UncertaintyBasis` for anything but display. | **Mechanical**: any new call site of `UncertaintyBasis::sample_size()` outside `posterior.rs`. |
| **LD-10** | **Rust and Android disagree on the sign of net interest from W2b onward.** Rust negates at extraction and keeps the sign; `DcfAnalysisEngine.kt:535-541` and `:802` still apply an unconditional `abs()`. This is a real divergence introduced deliberately by shipping one platform and not the other. | Juan's scope ruling of 2026-08-04 (§0.0): Windows first, Android ported once the Windows solution is good. Porting the behavioural half now would mean shipping a change to a platform whose model is not yet settled. | The Android port begins. | **Mechanical**, two one-line greps, either sufficient: (a) `DcfAnalysisEngine.kt:537` / `:802` still call `abs()` on interest; (b) the parity fixture still contains zero negative-interest rows. T2.12 plants the discriminating fixture row so (b) resolves itself the moment an Android export is regenerated. |
| **LD-11** | **The cross-platform parity suite is an exporter, not a check, and its comparator is wired to nothing.** Both `#[test]`s in `cross_platform_parity.rs` write JSON and assert only `path.is_file()` (`:319`) and `members.len() == 20` (`:513`) — **no value assertion anywhere**. Output goes to an untracked path (`:123-133`). The real comparison is `scripts/compare-windows-android-valuation-parity.ps1`, which nothing invokes but its own self-test. | Wiring a cross-platform comparator into CI requires an Android build in the loop — precisely what §0.0 defers. | The Android port begins, or any claim that "parity passes" is made about this suite. | **Mechanical**: `grep` for `compare-windows-android-valuation-parity` outside `scripts/` and the spec docs returns nothing. *[executed 2026-08-04]* |
...
| ~~Kotlin `SecDriverNormalizationPolicy.DriverOperator`~~ | **no change — §0.0 cuts T2.5.** *(This row was also self-contradictory in v3: it granted the silent default `List(qnames.size) { 1 }` that T2.5 itself forbade two sections later. Sensei r4 P0-6. Moot now, and struck rather than quietly corrected.)* | the hand-written Kotlin is untouched; adding a field to the **generated** type is compile-safe because its one consumer reads it by named field (`SecDriverNormalizationPolicy.kt:72-81`) and nothing destructures or `copy()`s it *[executed 2026-08-04]* |
...
| R2 | **Wave 2 moves published valuations. This is intended, not a risk to be disproved.** | High, and unmitigated by construction | v2 carried v1's inert-version answer here — that T2.6 *proves* no number moves. That proof died with Juan's Q1 ruling. The controls are: a pre-registered **published-effect** set (**`{MPWR}`**, −77 to −224 bps — measured by T2.0, no anchor) held separately from the extraction-incidence set (COF, DAL, CHTR, BKR, which move $0.00 published between them), sign-preservation tests for both filer classes, mandatory live QA with all five gate commands, and pause triggers (a′)/(c.1)/(c.2)/(e). **No wave may cite a proof of inertness for Wave 2.** |
...
| `apps/android/core/.../SecDriverNormalizationPolicyGenerated.kt` | | **W2a** | | | | *generated output only — the builder never hand-edits this file; it changes because the generator runs* |
| ~~`apps/android/core/.../SecDriverNormalizationPolicy.kt`~~ | | — | | | | **not touched — §0.0** |
| ~~`apps/android/app/.../SecEdgarTimeseriesProvider.kt`~~ | | — | | | | **not touched — §0.0** |
| ~~`apps/android/core/src/test/.../SecDriverNormalizationPolicyTest.kt`~~ | | — | | | | **not touched — §0.0** |
| `apps/windows/src-tauri/src/cross_platform_parity.rs` | | **W2a** | | | | T2.12 fixture row only |
...
| `_bmad-output/planning-artifacts/valuation-model-family-architecture.md` | | | | | R3 | *append `AD-VM-012`; do **not** create a new ADR file — F16b* |
| `docs/valuation-economic-contract.md` | | | | | R3 | *canonical home of the D7 register, per binding decision D7* |
...
will return zero. The sample is: the four anchors (**PG, GOOGL, AMZN, MSFT**), the Wave 2 issuers
(**COF, DAL, CHTR, BKR**, plus **MPWR** — *added in v6; it is the only issuer whose published
number Wave 2b moves, so a sample that omits it cannot observe the wave it is sampling*), and a
slice of the 26-name high-signal cohort chosen for the **oldest filing histories**, where missing
`filed` fields are most likely.
...
I1, I2, I3, I4, I5, I6 and I7 hold and are tested. All 31 construction sites carry real provenance or no longer
...
| Field | Content |
| --- | --- |
| **Wave id** | `wave-2`, **split into `wave-2a` and `wave-2b`** — **Round 2** |
...
| **Scope** | the driver-normalization contract, its generator, **both generated targets** (generated Kotlin gains the field inertly — §0.0), the Rust extraction site, the fingerprint sites, **the three `.abs()` sites in `dcf_model.rs` (LD-1)**, **the negative-interest guards in `driver_resolution.rs`**, the fixture-capture absence bug, one fixture row in `cross_platform_parity.rs`, and the contract documentation |
| **Out of scope** | the hand-written Kotlin policy and its consumer, any Kotlin test, any Gradle run — **§0.0** |
...
#### The split, and why the order is forced *(`ORCHESTRATOR-RULINGS.md` R-1)*

| | tasks | moves a published number? |
|---|---|---|
| **W2a** | T2.1, T2.2, T2.3, T2.4, T2.9, T2.12 — the contract, the generator, both generated targets, the Rust extraction sign, the fingerprints, the fixture row. **The three `.abs()` sites still stand.** | **No, and provably not.** Extraction negates, `.abs()` un-negates, the composition is the identity |
| **W2b** | T2.6 (LD-1), T2.7 (the ruling), T2.8, T2.10, T2.11 | **Yes, deliberately** |

W2a's inertness is the point: it makes W2a **reviewable against a fixed output**, so any number
that moves during W2a is by construction a mistake and needs no judgement to classify. W2b then
moves numbers deliberately, against a base already proven not to have moved them by accident.

> **Do not justify this split by claiming it protects `cross_platform_parity.rs` as a working
> instrument.** v3 said that and it is false — see **LD-11**: both of that file's `#[test]`s are
> exporters that assert no value at all, and the comparator that would check them is invoked by
> nothing. The split stands on the reviewability argument above, which does not depend on it.

> ### W2a and W2b are TWO review units and ONE merge unit. *(v6, R-10.3 — binding.)*
>
> T2.0's follow-up **measured** W2a's inertness rather than assuming it: twelve years of MPWR's
> interest were rewritten (`2014 +1.1M → −1.1M` … `2025 +29.2M → −29.2M`) and the model's own
> after-tax interest add-back did not move — **`Some(85)` → `Some(85)` bps of revenue**. Writing the
> field bypasses the setter's `.abs()` at `dcf_model.rs:907`, but **both read sites re-apply it**:
> `:551` (FCFF audit) and `:1590` (aligned driver bridge).
>
> That is the split working exactly as designed — and it is also the hazard. **W2a shipped alone
> leaves a contract, a bumped fingerprint `/9`, and generated policy in both languages all declaring
> a sign convention that the code does not honour** — the precise "generator/output drift" shape the
> contract pipeline exists to prevent, installed deliberately.
>
> Therefore: **W2a may be reviewed separately, but must not be merged without W2b.** If W2b stops —
> for a pause trigger or any other reason — **W2a does not merge either**; both wait. A builder or
> reviewer that proposes landing W2a alone "because it is inert" has inverted the argument: its
> inertness is exactly what makes it unsafe to ship by itself.

#### T2.0 — PRE-FLIGHT, BLOCKING. Measure the filed sign before building anything.

*(New in v5, and it blocks W2a. Sensei r5 question 2 / P0-8.)*

**Nobody has measured which issuers file net interest as income and which as expense — and the whole
wave is pre-registered on the answer.** Two incompatible traces were argued this session on the same
unmeasured premise:

- the orchestrator's: COF files positive, DAL/CHTR/BKR file negative → the guard fires only for COF;
- the Sensei's, on the same assumption: **DAL/CHTR/BKR are bit-identical before and after.** A net-expense
  filer files `−X`; today `.abs()` → `+X`; after W2b, negate → `+X`, no abs → `+X`. **No change at all.**

The arithmetic is right. The premise under both is not established anywhere. `brief.md:30-32` says only
that `InterestIncomeExpenseNet` *"resolves as an expense"* for all four — which is a statement about the
value **after** `.abs()`, and therefore says nothing about the filed sign. And it cannot be recovered
from fixtures: **every valuation fixture in the tree contains zero negative interest values** —
`baseline_driver_data_2026-07-30.json` → 0, `core_driver_data_deep.json` → 0 *[executed 2026-08-04]* —
because all captured data is already post-`.abs()`. The evidence was destroyed at capture time by the
very defect this wave removes.

**If the Sensei's trace is right, W2b's predicted published-number change is: nothing, anywhere.** COF
dispatches to `residual_income`; DAL/CHTR/BKR are inert; and the affected class — issuers that are
`OperatingNonFinancial` **and** file net interest *income* — **is named nowhere in this plan.** The
wave would then be shipping a fix demonstrated on zero issuers while calling itself the run's riskiest.

**So measure it first.** A network probe in `valuation_probes.rs`, `#[ignore]`, diagnostic only,
following the `probe_growth_persistence_rho1` convention, over the 26-name cohort plus COF, DAL, CHTR,
BKR and BAC. For each issuer-year it reports, from **raw companyfacts before any negation or `.abs()`**:

| column | why |
|---|---|
| the winning interest qname | which concept actually resolved |
| the **filed sign** | the fact the entire pre-registration rests on |
| `BusinessClass` | `OperatingNonFinancial` vs `FinancialServices` — decides whether a change can reach a published number |
| guard outcome under the new convention | fires / does not fire |
| predicted published delta | zero, or a magnitude |

The cheaper equivalent the Sensei proposed also works and needs no new probe: **run extraction twice
over the cohort — once with `negatedQnames` populated, once with those two qnames removed — and diff
the interest series per issuer.** The difference set *is* the affected set, computed from the contract,
today, using the generator's existing `-OutputRoot` machinery. Either instrument is acceptable; the
output is what matters.

**This single measurement discharges four things at once**, which is why it is worth blocking on:
1. the pre-registered affected set (§6.2 currently claims a magnitude it never states);
2. **LD-8's exposed set** — and with it Juan's binding condition, since the difference set *is* the
   detector LD-8 says it does not have;
3. pause trigger (c)'s class, which P0-8 shows is currently set on the wrong set;
4. whether W2b moves any published number at all.

**Exit:** the affected set is enumerated by name, or **declared empty with the evidence**. Declaring it
empty is a legitimate outcome and is **not** a reason to weaken anything — but it must be stated, because
a wave that changes no published number may not describe itself as the run's riskiest, and §6.2's
"direction, magnitude and affected-issuer set" claim must then be rewritten to say so.

> ### ✅ T2.0 EXECUTED — 2026-08-04. Report: `build/t2.0-filed-sign-measurement.md` (+ `-raw.txt`)
>
> **The measurement contradicted both traces the plan argued.** Enumerated, not empty — but not the
> set the plan registered. All four discharged items resolved:
>
> 1. **Affected set (published effect) = `{MPWR}`** — Monolithic Power Systems, 12 consecutive net
>    years, **inside the 26-name gate cohort**, named in no prior revision. §6.2 corrected.
> 2. **LD-8's exposed set confirmed `{BKR, CHTR, DAL}`** — MPWR would be a naive fourth but has **no
>    filed debt in any year**, so its net years never reach the cost-of-debt fit. LD-8 stands.
> 3. **Trigger (c) was set on the wrong quantity**, not merely the wrong set — it keyed on refusal,
>    and the only issuer that moves never refuses. Split into (c.1)/(c.2) above.
> 4. **W2b moves exactly one published number** in these 29 names — MPWR's, **−77 to −224 bps every
>    net year** (−169 bps, −$47.3M in 2025). COF **$0.00**; DAL/CHTR/BKR **bit-identical**; anchors
>    PG/GOOGL/AMZN/MSFT net = 0, delta 0.
>
> Instrument validated by control: **BAC `+60,096M` for 2025**, matching `brief.md` exactly. The
> zeroes are a result, not a blind probe. Probe `probe_filed_interest_sign` is `#[ignore]`, asserts
> nothing, `rustfmt --check` clean.
>
> **Still open, ordered on the same builder, not blocking W2a:** (i) does MPWR **cross side of its
> gate** — the measurement that decides whether trigger (b) fires and Wave 2b stops for Juan;
> (ii) the **wider scan** beyond 29 names, since the net-cash/debt-free archetype is common and the
> "exactly one" claim is cohort-local until it returns.
>
> **Builder judgement call to honour when T2.1 lands:** the probe's `PROPOSED_NEGATED_QNAMES`
> duplicates the two strings T2.1 adds to the contract. On landing, replace it with a read of the
> generated `qname_signs` so the probe cannot drift from the policy it is testing.

**Restated pause trigger (c)** — replacing the version keyed on the flat set {COF, DAL, CHTR, BKR},
which tolerates the observation that would falsify the ruling and fires on none of the predicted ones.

**T2.0 has now executed; the third row is measured.** Three reproducible runs against SEC
`companyconcept`, control BAC `+60,096M` for 2025 matching `brief.md` to the dollar — so the zeroes
below are a result, not a dead instrument. Report: `build/t2.0-filed-sign-measurement.md`.

| measured | stop condition |
|---|---|
| guard **fires**: COF only (2024, 2025), published delta **$0.00** — `FinancialServices` → `residual_income`, `fcf_history` never passed | **any other issuer whose guard fires is a stop** |
| guard **never fires**, numbers unchanged: DAL, CHTR, BKR — bit-identical, every `dFCFF` exactly 0 | **any non-zero published delta for these three is a stop** — the arithmetic says zero |
| guard never fires, **numbers change: `{MPWR}`** — 12 consecutive net years 2014-2025, filed `+1.1M`…`+29.2M`, **no filed debt in any year** so the net years never reach the cost-of-debt fit | **−77 to −224 bps of revenue is the predicted band.** A result outside that band, or a *positive* shift, is a stop |

**The condition that was missing, and is the reason T2.0 was worth blocking on** *(R-7.2)*:
(c) as previously written keyed entirely on **refusal of the cost-of-debt channel** — on the guard
firing. MPWR proves a published number can move while the guard **never** fires. The trigger was
blind to the only issuer the change affects. Added, needing no invented threshold because the
prediction for every other name is exactly zero:

> **Any issuer outside the pre-registered published-effect set `{MPWR}` with a non-zero published
> delta is a stop** — whether or not its guard fires.

**The direction is correct; do not treat it as a regression** *(R-7.3)*. `OCF` already contains
interest received. The FCFF bridge adds back `interest × (1 − t)`. For a net-interest-**income**
filer, `.abs()` presents that income as an expense and the bridge adds it back, **double-counting
income the OCF already carried**. MPWR's published FCFF has been **overstated for twelve years**;
the correction makes the bridge subtract and FCFF falls to its true level. **No threshold moves to
keep MPWR passing, no ticker special-case, and the lost 224 bps is not recovered through another
channel.** If MPWR leaves the gate cohort, the cohort was wrong, not the correction.

#### W2a — Done-when *(new in v5; Sensei r5 P0-2)*

v4 split the wave and gave W2a no exit, no baseline and no measurement — so its inertness was
*asserted rather than observed*, which is the same shape as v2's invariance proof that Juan's Q1
ruling already killed. An unmeasured inert wave is worse than no split, because it divides review
attention without producing the fixed output the split exists to review against.

1. **J1, J2, J3, J4, J5 and J10 hold.** **J6 must be FALSE at W2a's exit** — the three `.abs()` sites
   are still standing, deliberately. A W2a that satisfies J6 has done W2b's work and broken the split.
2. The `/9` bump is in all five fingerprint sites (T2.9's table); the generator's `-Check` is green
   for **both** targets.
3. **A live per-issuer table, measured, not argued** — the same issuer set as W2b (the four, plus BAC,
   plus the 26-name cohort), from cached payloads, with a retrieval timestamp. **Every delta exactly
   zero.** This table is W2a's whole reason for existing, and it becomes W2b's "before".
4. All five mandatory gate commands, as a delta against the baseline W2a captured at its own start
   (R-4), with the protected failing set named and never counted.
5. **Pre-condition, executed before W2a starts:** the inertness argument is exhaustive only for
   consumers that read `FcfPoint.interest_expense_dollars`, because the setter at `:907` abs-es on
   write. It says nothing about a consumer reading the *extractor's* local before it becomes a field.
   Settle it: `Select-String -Path src\*.rs -Pattern 'extract_driver_annual|extract_annual_equivalents'`,
   and read `edgar.rs:1060-1095` for every use of that local. Any second consumer is a finding.
...
**T2.5 — ~~The Kotlin path applies the same sign.~~ CUT by §0.0. Do not implement.**

Android is out of scope. The hand-written `SecDriverNormalizationPolicy.kt` does **not** gain
`qnameSigns`, `SecEdgarTimeseriesProvider.kt` does **not** negate, and no Kotlin test is written.
The *generated* Kotlin still gains the field — inertly, and verified compile-safe in §0.0 — because
narrowing the generator to one target would manufacture the drift §1 exists to repair.

Consequence, registered as **LD-10**: from W2b onward Rust and Android disagree on the sign of net
interest. Deliberate, Juan's call, detector attached.

**T2.11 — ~~Fix `KotlinList`'s empty-collection defect.~~ CUT by §0.0. Do not implement.**
*(This is the T2.11 of §1.5's D6 discussion, not the documentation task numbered T2.11 below —
the duplicate id is itself corrected in v4; see the changelog.)* The defect emits a stray leading
comma inside `listOf(` and affects **Kotlin output only**; Windows is unaffected. Latent — no driver
has empty qnames, and `qname_signs` cannot be empty because every driver has qnames. Deferred to
the port with the rest of the Android work.

**T2.12 — Plant the discriminating fixture row, so the divergence detects itself later.**
*(New in v4. Windows-only; no Kotlin build, no Gradle.)*

`baseline_driver_data_2026-07-30.json` contains **zero** negative-interest rows —
`(Select-String -Path <fixture> -Pattern '"interest":\s*-').Count` → `0` *[executed 2026-08-04]*.
So even a fully wired comparator would compare on inputs that cannot discriminate the sign
convention. Add **one** row carrying a negative interest value.

**Use MPWR's real filed series, not an invented number** *(v6, from T2.0)*. When this task was
written no measured net-interest-income series existed in the plan, so a synthetic row was the only
option. T2.0 measured one: MPWR files `InterestIncomeExpenseNonoperatingNet` positive for twelve
consecutive years (**2014 `+1.1M` … 2025 `+29.2M`**, `build/t2.0-filed-sign-measurement-raw.txt`).
A fixture row built from a real filing discriminates the same convention while also being
falsifiable against EDGAR — a synthetic value can only ever be self-consistent. It is also the
issuer whose published number the wave actually moves, which is the case a port most needs to
reproduce.

This does not complete a comparison today — there is no Android export to compare against while
§0.0 holds. It means the discriminating case is *already present* when someone regenerates the
Android export at port time, so **the existing instrument catches LD-10 on its first run** instead
of depending on a future engineer remembering to construct the case.

*Acceptance:* the fixture parses; `export_qa_cohort_parity_snapshot_for_android` still writes;
the exported JSON contains the negative-interest row. **Do not claim this establishes parity** —
per LD-11 the suite asserts no values, and per §6.3's stale-fixture pattern, any report of a green
run here must state that it is Windows-only evidence and not cross-platform agreement.

<details><summary>Superseded T2.5 text, retained for the record</summary>
...
*Acceptance:* all three tests pass; **the canonical check named in J6** (PowerShell `Select-String -Pattern 'interest.*(abs\(\)|f64::abs)'`) returns nothing — **run the canonical check by reference, never retype a pattern here** (Sensei r5 P0-3: this exact line shipped the blind pattern through three revisions);
...
| `interest < 0` (any debt) | silently drop the **year** | **refuse the channel for that issuer.** ~~fall through to the other cost-of-debt path~~ — **there is no other path; see below** | a net-negative year proves the series is net; a rate fitted on the surviving years is biased upward. Refusing is Decision 1 applied |
| `interest == 0`, `debt > 0` | dropped by `interest > 0` | unchanged | out of scope to re-adjudicate; recorded as **LD-7** |

**Correction — "falls through to the other cost-of-debt path" was false, and it is the load-bearing
sentence of this whole task.** *(Sensei r4 P0-2.)* Refusing the accounting channel does not degrade
to another rung. It terminates:

- `edgar.rs:1099-1100` passes `None, None` for both `market_yield_bps` and
  `rated_or_synthetic_spread_bps`, so neither alternative rung ever carries a value;
- `driver_resolution.rs:236-239` then returns a terminal
  `Err("fcff unavailable: no aligned market yield, spread, or SEC interest/debt periods")`.

**The two alternative rungs have no producer anywhere in the tree.** All 12 mentions of
`market_yield_bps` / `rated_or_synthetic_spread_bps` are a declaration, a `None` default
(`dcf_model.rs:889-890`), a setter (`:940-941`) and two consumers (`driver_resolution.rs:95`,
`:106`). A search for any bond-yield, credit-rating or synthetic-spread provider returns only those
same field names. *[executed 2026-08-04]* So **channel refusal is issuer blackout** on the FCFF
path, and building the missing rung is not available — it would require either a new market-data
integration (out of scope) or synthesizing a spread from an assumption, which is *selecting an
estimator* and is forbidden outright.

**Who is actually exposed, measured rather than assumed** *(`ORCHESTRATOR-RULINGS.md` R-5)*:

| issuer | filed | dispatch | consequence of the refusal |
|---|---|---|---|
| **COF** | net interest **income** (positive) → negated → negative → guard fires | `FinancialServices` → `residual_income` (`dcf_model.rs:1263-1265`); `fcf_history` is **not passed** | **Its published valuation does not move at all.** The guard reaches only `resolve_attribution_wacc`, whose sole caller is `valuation_gap_attribution.rs:1722`, whose `Err` arm (`:1742-1744`) writes a `source_note` and continues — inside a diagnostic the file itself disclaims (`:32`, `:146`, `:1325`). Its e2e gate asserts `residual_income_equity` and stays green (`cof-detail.native.e2e.mjs:83,88`) |
| **DAL, CHTR, BKR** | net interest **expense** (negative) → negated → positive → guard never fires | `OperatingNonFinancial` → `fcff_wacc` → `derive_wacc` | fitted as gross when the series is net; cost of debt understated, numbers plausible. **This is LD-8's real exposed set** |

**Ruling, split by class because the two are not the same defect.**

**COF: accept the refusal.** Decision 1 applies, and applies *cheaply* — no coverage is lost to weigh
against it, because COF's published number comes from `residual_income` and does not move.

**DAL/CHTR/BKR: ship the sign fix; carry the net-vs-gross residual as LD-8.** The ground for this is
**not** "refusing them would discard coverage," which is how v4 argued it. *(Sensei r5 question 2:
that argument brushes against Juan's closing constraint for no reason, and a stronger one is
available.)* The stronger ground is that **this wave does not change these three at all.** A
net-expense filer files `−X`; today `.abs()` → `+X`; after W2b, negate → `+X`, no abs → `+X`.
Bit-identical, and T2.7's own table says the guard never fires for them. So there was never a
fix-versus-refusal *choice* here — refusing them would be a **new** refusal of currently-published
issuers, on the basis of a defect (net basis fitted as gross) that predates this wave and that this
wave neither creates nor touches. **That is scope expansion, not Decision 1 applied**, and it needs
no appeal to coverage to reject.

> **Both statements above are conditional on T2.0, and T2.0 has not run.** The filed sign per issuer
> is unmeasured (see T2.0). If T2.0 shows DAL, CHTR or BKR filing net *income* rather than net
> expense, the class assignment here is wrong and this ruling reopens for that name. That is exactly
> what pause trigger (c) is now keyed to detect.

Precedent, for the shape of the answer rather than for its content: this project already ran the
all-refuse alternative. The credit-curve episode went 15-of-20 dark
(`prd-discount_screener-2026-08-03/.memlog.md:26-29`) and was resolved by fixing five real evidence
bugs and *keeping* real evidence — once by falling back to the newest **complete** filed year — never
by fabricating a rung and never by accepting the cascade.

**LD-8's "no mechanical detector" is an overclaim, and T2.0 supplies the detector.** *(Sensei r5,
strongest recommendation of the round.)* The register says the correct rule keys on the *basis* of the
series and is unimplementable until `FcfPoint` carries per-field provenance — item 6, out of scope.
That is false as a claim about *detection*: running extraction twice, with and without `negatedQnames`,
and diffing the interest series per issuer yields the exposed set today, from the contract, with no
`FcfPoint` change at all. LD-8 must therefore either carry that detector or state explicitly that the
cheap detector was priced and declined — *"we could not"* is not accurate and does not meet D7's own
standard.
...
*Acceptance.* Four assertions, and the first two are not the same test:

1. **Branch-direct**: constructing a genuinely negative `interest_expense_dollars` at the
   `resolve_rate_inputs` boundary yields **no** accounting cost of debt — not one fitted on the
   surviving years.
2. **Issuer-level outcome**, which the year-level test cannot show: an issuer with one
   negative-interest year ends with the FCFF path returning the terminal `Err`, i.e. *blackout*,
   not a quietly degraded rate. Assert the outcome, not the intermediate `None`.
3. The all-positive case is bit-identical to today.
4. **COF's published-value delta is exactly $0.00**, turning the structural argument above into a
   measured one.

The refusal-rate change is measured and reported, never assumed. **W2b must additionally report
DAL/CHTR/BKR's largest published-value swing as a percentage of current published value** — that is
the one number that could reopen this ruling. Small swings and no anchor-adjacent gate flip: LD-8
stands. A large swing: *that name* gets the refusal treatment, not the whole ruling.
...
**T2.14 — The alignment check D6 mandates and no task delivered.** *(New in v5; Sensei r5 P0-7. Belongs to W2a.)*

Binding decision **D6** requires *"the **alignment** check that a length check cannot give: a
generated-code test that **reconstructs** the sign array from `negatedQnames` and compares. J1 catches
truncation; only reconstruction catches a wrong value at a right length."* **No task wrote it.** Not
T2.2, not T2.3, not T2.4; no invariant J1–J10 contains it. The nearest example row, W2-P03, asserts
*"all ones except the two declared"* — a **hard-coded expectation of two positions**, which is exactly
what the superseded T2.5 acceptance warned against (*"by looking the index up from `qnames`, never by
hard-coding a position"*).

So the one defect the positional-array design is uniquely exposed to — a **right-length, wrong-value**
array — has no check anywhere. A silent transposition of the two negated indices passes J1, passes
W2-P03's count, and produces exactly inverted economics.

Write a generated-code test that, **driver by driver**, reconstructs the expected sign array from the
contract's `qnames` × `negatedQnames` and compares it element-wise to the generated constant. Look
every index up from `qnames`; hard-code no position and no count.

*Acceptance:* the test fails when any single sign is flipped, and when the two negated indices are
transposed. **Observe both failures before relying on it** (third clause). Add example row **W2-P09**
for the reconstruction, and correct W2-P03 to locate its positions by lookup rather than asserting
"the two declared".

**T2.13 — Documentation.** *(Renumbered in v5. v4 left two tasks numbered T2.11 while claiming above that the duplicate was corrected — Sensei r5 P0-6.)*
...
~~Fix `KotlinList`'s empty case anyway~~ — **struck by §0.0**, see D6. Deferred to the Android port.
...
- **J10** *(new in v3; renumbered from a colliding second `J7` — the id was already taken by the
  inherited sign-non-invariance rule, which made "Done when: J1 through J9" ambiguous)*
  **Every `with_operating_drivers` call site in the tree has been enumerated
  and classified before the wave completes.** Removing the setter's `.abs()` changes the *meaning of
  the inputs* of every one of them, production and test alike — a fixture that passes a negative
  was, until this wave, silently receiving a positive. The wave searched for the sites that
  *performed* the abs; **J10 is the search for the sites that depended on it.** *(v4 still called this invariant J7 in its own body after the r4 renumber — Sensei r5 P0-6.)* Enumerate with
  PowerShell from `apps/windows/src-tauri`:

  ```powershell
  Select-String -Path src\*.rs,tests\*.rs -Pattern 'with_operating_drivers' |
    ForEach-Object { "{0}:{1}" -f $_.Filename, $_.LineNumber }
  ```

  Each site is classified **unaffected** (all interest arguments non-negative, so abs was a no-op)
  or **affected** (a negative can reach it). Every affected site is listed in the wave report with
  its classification and its evidence. An affected site inside a file §2.0 declares untouchable —
  `valuation_baseline.rs:900` is the known one — is a **stop-and-report under trigger (e)**, never a
  fixture edit.

  **Executed before being written here (v3 second clause). It returns 45 sites across 5 files:**

  | File | Sites | Note |
  | --- | --- | --- |
  | `dcf_model.rs` | 23 | `:896` is the setter itself; the rest are its own tests |
  | `valuation_baseline.rs` | 11 | the published-value baseline — §2.0 untouchable |
  | `cross_platform_parity.rs` | **9** | **not named by any reviewer or by v2** |
  | `driver_resolution.rs` | 1 | `:328`, the `point()` helper behind all 10 of its tests |
  | `edgar.rs` | 1 | `:1089`, the sole production site |

  **The surface is 45, not one.** v2's discussion of this hazard named a single site. Two facts
  follow that change the wave's shape. First, `cross_platform_parity.rs` is where Rust/Kotlin parity
  is asserted, and this plan already carries "parity is unmeasured while Wave 2 moves both
  platforms" as a standing risk — nine of its fixtures are built through the abs'd setter, so the
  parity suite is **inside** the blast radius rather than a check on it. Second, one production site
  and forty-four test sites means the classification work is almost entirely fixture archaeology,
  and a builder that finds it tedious will be tempted to run the suite and fix what turns red. **That
  is the forbidden move.** The classification is the deliverable; a green suite is not evidence that
  the inputs still mean what their author intended.
- **J6** All three `.abs()` sites are removed together. The acceptance search is **PowerShell**,
  run from `apps/windows/src-tauri/src`, and it must return nothing:

  ```powershell
  Select-String -Path dcf_model.rs -Pattern 'interest.*(abs\(\)|f64::abs)'
  ```

  **Executed against the known-positive case before being written here** (v3's second clause).
  Against the current tree it returns exactly three rows — `:551`, `:907`, `:1590` — so it is known
  to fire when it should. v2 shipped `grep -nE "interest.*(abs\(\)\|f64::abs)"` instead; under `-E`
  the `\|` is a **literal pipe**, and the same execution returns **0 rows with all three sites
  intact**. A builder would have seen a clean check and shipped the defect. Markdown pipe-escaping
  inside a table cell is the mechanism, and it renders looking correct.
...
| ~~W2-P04~~ | — | ~~Kotlin policy~~ | **CUT by §0.0.** Its intent — that the *generated* interest-expense operator carries negative signs at exactly the two net-concept indices — survives without a Kotlin test: the generator's `-Check` mode, wired into `validate-contracts.ps1` by §1, proves committed output equals generated output for **both** targets, and runs on Windows with no Gradle | | | |
| ~~W2-P05~~ | — | ~~Kotlin provider~~ | **CUT by §0.0.** This one asserted Android *behaviour* and has no Windows-side substitute. Its absence is exactly what **LD-10** records | | | |
| **W2-P06** | positive | parity fixture (T2.12) | the fixture row carrying a negative interest value | `export_qa_cohort_parity_snapshot_for_android` runs | the exported JSON contains the negative-interest row — the discriminating case is present for the eventual port. **Not a parity claim** (LD-11) | LD-10 detector (b) |
...
| W2-P07 | positive | FCFF bridge | a LIN-type filing: net **expense**, filed negative, negated by the convention | the bridge runs | it arrives as a **positive** expense | T2.6 test 1 |
...
frozen fixture corpus test; ~~one Kotlin unit test~~ (**cut — §0.0**); the PowerShell contract validator; **and a full
...
| search | **J6's PowerShell `Select-String`, copied from J6 — not retyped, not simplified.** Returns nothing when the wave is done; returns three rows today. The naive `interest.*abs()` is blind to the setter at `:907` and a `grep -E` transcription is blind to everything. |
...
| `cargo test --lib quant_lens::` | **Justification corrected in v3 — the old one was false.** `quant_lens.rs` does **not** read the driver series: the exhaustive read-side sweep for `interest_expense_dollars` returns 15 sites across 6 files and `quant_lens.rs` is not among them. It reads the FCFF **output** — `fcff_candidate.intrinsic_value_cents` (`:364`), `normalized_fcff_dollars` (`:630`), `fcff_base_cents` (`:385`). That is a better reason to gate on it, not a worse one: Wave 2 moves published FCFF, and this is a consumer of published FCFF. The blast-radius table is right to exclude it and the gate is right to include it; only the stated reason was wrong. |
...
| `cargo test --lib valuation_gap_attribution::` | **Added in v3.** `resolve_rate_inputs_for_source` → `derive_wacc` has **two** callers, not one: `fcff_wacc` and `resolve_attribution_wacc` (`dcf_model.rs:608-646`), whose sole workspace caller is `valuation_gap_attribution.rs:1722` — in a path commented *"CHTR-class ordering failures"*, the exact issuer class this wave measures. That file has 15 tests and **none of the other four commands runs them.** |

**Why this one was missing, and the lesson that outlives it.** v2's gate list was assembled by
checking v1's omissions rather than by walking the call graph of the function T2.7 changes. The
list was "restored in full" against the wrong reference. Any future wave that changes a function
derives its gate from that function's callers, transitively — not from what a previous revision
forgot.

Also: `cargo test --lib` (full Shell suite). ~~and the Android unit tests (Gradle) for T2.5~~ —
**no Gradle run; T2.5 is cut by §0.0.** The generated Kotlin's correctness is locked instead by the
generator's `-Check` mode inside `validate-contracts.ps1`, which is PowerShell and Windows-runnable.
...
- **(a′)** **any anchor goes unavailable. Unconditionally — the T2.7 exemption in (b) does NOT
  apply to an anchor.** This is the single most important line in the wave. T2.7's refusal is not
  contained to the cost-of-debt channel: `edgar.rs:1099-1100` supplies `None, None` for the only
  two alternative rungs, so an emptied accounting rung falls to the terminal `else` at
  `driver_resolution.rs:236-239` and returns `Err`, which propagates through `derive_wacc` and
  `fcff_wacc`. **Channel refusal is issuer blackout.** A builder that sees an anchor go dark and
  classifies it as "declared refusal, therefore expected" has evaded Juan's pause condition by
  construction. Stop.
- **(b)** an operating issuer goes from valued to unavailable, or the reverse, other than through
  T2.7's declared refusal — **and never for an anchor, where (a′) governs with no exemption.**
- **(c)** *(two conditions, disjunctive — either alone stops the wave; see R-7.2)*
  - **(c.1) refusal channel.** The accounting cost-of-debt channel refuses for any issuer outside
    the **extraction-incidence** set — **COF, DAL, CHTR, BKR** — which would mean net-filing is more
    widespread than the measurement suggests.
  - **(c.2) published effect.** *Re-keyed **twice**: `{MPWR}` (cohort-local) → the 25-symbol
    driver-level set (R-10 scan) → **the 6-issuer set the counterfactual measured actually moves a
    published number** (R-13). The 25-symbol version would have fired on nineteen true negatives —
    issuers whose interest series changes and whose published value does not.* A stop is any of:
    - **any issuer outside `{ROST, MPWR, JKHY, ULTA, CPRT, NKE}` with a non-zero published delta**,
      *whether or not its guard fires* — including any of the other 19 driver-level affected names;
    - **any move upward**, or **any move beyond −157 bps** (the measured maximum, ROST);
    - **any router-lane flip**, anywhere. The counterfactual measured **zero**, MPWR included —
      this is qualitative and outranks magnitude;
    - **any anchor moving at all.** All four measured $0.00 at `yrs=0` — not one fiscal year
      differs — so any anchor movement means W2b did something the deletion did not;
    - **any corrected FCFF going non-positive.** The counterfactual produced none. CARR, DDOG, NWS,
      NWSA, TTD have no candidate on either side and are pre-existing absence, not this wave's
      refusal.

    **(c.1) alone cannot fire on MPWR** — it has no filed debt, so its guard never fires while its
    published value moves −357¢. T2.0 measured exactly that case, and without (c.2) the wave's real
    effect is invisible to its own trigger.

  Both sets are the prediction; a miss is a finding, not a tolerance. **A prediction that all 25
  move and a prediction that none do are both falsified by this wave** — that is what makes the
  six-name registration a real one.
- **(d)** any of the three protected failing tests changes state (see *Done when*).
- **(e)** **removing the `.abs()` changes the meaning of an existing test's inputs.** Every test in
  the tree that builds an `FcfPoint` through `with_operating_drivers` was, until this wave, feeding
  values that were silently abs'd; the fixture author may have written a negative expecting a
  positive. The wave searches for the sites that *depended on* the abs, not only the sites that
  *performed* it. **`valuation_baseline.rs:900` is such a read site and §2.0 declares that file
  untouchable** — if it fails, the wave is deadlocked between "no fourth failing test" and "no wave
  may edit this file". **That deadlock is a stop-and-report to Juan, never a test edit.** Changing a
  fixture to match new behaviour is indistinguishable from weakening a check, and the standing rule
  forbids it.

#### Evidence of pass

The names and results of T2.6's three sign-preservation tests and T2.7's two guard tests, pasted
verbatim. All **five** mandatory gate commands with their output. The per-issuer live table including
COF and the 26-name cohort. The live QA record with actual commands and results. The scratch-root
generator diff. The `validate-contracts.ps1` output. ~~The Kotlin test name.~~ (**cut — §0.0; this evidence item cannot be produced**). The
`sec-driver-normalization/8` and the **canonical J6 check's** output — before and after, per the third clause. `git diff --name-only`.
...
**J1, J2, J3, J4, J5, J6, J7, J8, J9 and J10** hold — enumerated, not a range. **J10 is the 45-site
classification and it was outside v3's first exit range**, which is the defect this enumeration
exists to prevent: an invariant appended to a list whose exit criterion was never widened is an
invariant a wave can be declared done without. The `/9` bump is in all five sites and two of them
came from the generator.
All three `.abs()` sites are gone. T2.6's and T2.7's tests pass and are quoted in the completion
statement. All **five** mandatory gate commands ran and are recorded with their output. Live QA ran in
...
refuse where the naked mean published (R4). Count, over the **20-name** pinned cohort using the
...
**`fuse`'s arithmetic, read and pinned in v3. Sensei r3 P0-1 is closed, and the answer is the
opposite of the concern.** P0-1 asked whether passing an already-divided `variance_of_centre()`
*plus* `observations = retained()` would make `fuse` divide twice and overstate precision by a
factor of ~n — a larger weighting error than the one being removed, introduced by the fix, in the
channel the fix is about. It would have been the most damaging possible outcome, and nobody had
read the function. It has now been read:

- `fuse` weights by `Observation::precision()` (`posterior.rs:72`), which is
  `1.0 / uncertainty.variance()` and nothing else (`evidence.rs:100-102`).
- `observations` enters the arithmetic **nowhere**. Its only use is `measured_sample_size()`
  (`posterior.rs:119-125`), which sums sample sizes across channels purely to *label* the fused
  posterior's basis. It touches neither `estimate` nor `posterior_variance`.

**The variance field IS the weight.** Part 2 supplies the weight and part 3 supplies a label, so
they cannot conflict and there is no double division. T3.8 as specified is correct.

Two consequences a builder must respect. First, this closes the question — do not re-derive it, and
do not "defensively" un-divide `variance_of_centre()` to compensate for a division that does not
happen; that would understate precision by ~n in the same channel. Second, `Uncertainty::from_variance`
returns `None` for a non-positive variance (`evidence.rs:87-89`), so a retained set with zero spread
makes the channel **absent** rather than infinitely precise. That is correct behaviour and **K10**
pins it: trimming to a set of identical values yields an absent trailing channel, not a divide-by-zero
and not a certainty. The labelling defect found while reading this is recorded as **LD-9**.
...
- The old-versus-new diagnostic table: `growth_persistence` (today `0.1709`), `fade_per_year`,
  ...
  **20-name** cohort.
  ...
  > **The pinned cohort is 20 names, not 28** *(corrected in v6; Wave 3's builder measured it,
  > `core_driver_data_deep.json` → `rows` has exactly 20 keys)*. Composition:
  > `AAPL ADMA AMSC AMZN APP BWMN CALX FIGS HURN IDCC INOD INVA MH MIR MSFT ROCK T VICR VRRM VRT`.
  >
  > **It is disjoint from Wave 2's issuers.** It contains **no MPWR**, and none of COF/DAL/CHTR/BKR.
  > So Wave 3's *"refusal-rate change is zero"* result is evidence about **this** cohort and says
  > **nothing** about the issuers Wave 2 moves. The two cohorts must not be conflated in any report:
  > the 26-name **high-signal gate** cohort (which contains MPWR and COF) and this 20-name **pinned
  > measurement** cohort are different populations serving different purposes.
...
**K1, K2, K3, K4, K5, K6, K7, K8, K9 and K10** hold and are tested — enumerated, not a range.
**K9 was outside the old `K1 through K8` range, and K9 is where Juan's Q2 ruling lives**: no naked
mean or variance at `:280`/`:536`, and `observations` reporting the retained count. Wave 3 could
have been declared done with the ruling unimplemented. K10 is v3's zero-spread pin. The audit doc's
eight rows each carry a reason. T3.7's number
...
**T5.9 — The ADR.**
**Location corrected in v4 — record it as `AD-VM-012` in the existing series, not as a new file.**
*(Advisor r4 P1-2, verified at F16b.)* v3 created
`docs/adr-0001-fr-29-removal-and-explicit-unavailable-state.md` with fresh `ADR-000#` numbering and
a new `## Architecture Decisions` section in `docs/index.md`, on the strength of F16's "docs/
contains no ADR". But `_bmad-output/planning-artifacts/valuation-model-family-architecture.md`
already carries `AD-VM-001` … `AD-VM-011` and `AGENTS.md:590` already labels it *"valuation ADRs"*.
FR-29 is a valuation decision; splitting the valuation decision record across two conventions is
exactly the fragmentation the documentation-map discipline exists to prevent.

So: **append `AD-VM-012` to `valuation-model-family-architecture.md`**, matching the heading shape
of its siblings; do **not** create the new file and do **not** add the new `docs/index.md` section.
`AD-VM-003` — *"Financial Services Use Residual Income / Excess Return on Equity"* — is the
sibling this decision sits next to, and is the same dispatch T2.7's ruling turns on.

**The register's home is not conditional, and v4 made it look conditional.** *(Advisor r5 finding 2.)*
Binding decision **D7** already says, unconditionally: *"The register lives in
`docs/valuation-economic-contract.md` and is linked from the ADR."* v4's T5.9 then required the ADR
to *contain* the register, with the economic-contract location as a fallback only if a heading shape
could not carry it. **Those cannot both be followed.** D7 wins — it is binding and builders may not
re-open it.

So: **`AD-VM-012` links to and briefly summarizes the register; the full table stays in
`docs/valuation-economic-contract.md`.** The heading-shape question is moot and struck — the
Advisor verified that AD-VM-002 and AD-VM-011 already carry multi-row tables under the plain
`### AD-VM-0NN: Title` heading, so capacity was never the real risk.

The correctness reason matters more than the formatting one: the register is **living** — D7 assigns
its ownership to *"this plan's successor run"*, so it keeps growing after this plan ends. A growing
register does not belong embedded in a point-in-time decision record that later waves would have to
keep editing.

The decision record must contain, in the standard shape (context, decision, consequences,
alternatives, status), and none of these is optional:
...
- **a link to the D7 latent-defect register in `docs/valuation-economic-contract.md`, plus a
  one-paragraph summary — not the table.** The register itself is written there, enumerated by id:
  LD-2, LD-3, LD-4, LD-5, LD-6, LD-7, LD-8, LD-9, LD-10, LD-11 (LD-1 struck as closed by Wave 2),
  **each with its owner, trigger condition and detector.** *(v3 wrote "LD-1 through LD-7" while the
  register already ran to LD-9; a range that silently excludes the newest entries lets a wave be
  declared done without them — Sensei r4/r5. Placement corrected per D7 — Advisor r5.)*
...
**T5.12 — MOVED TO PRE-FLIGHT (§4.1). Not a Wave 5 task in v3.**
Retained here only so a reader of Wave 5 is not left looking for it. It ran before Round 1, because
the exemption it proves is spent by Wave 3 in Round 1 — two rounds before Wave 5 would have proven
it. If §4.1 recorded a failure, Wave 5 has full live QA and the *Live QA* subsection below does not
apply. Original statement follows.
...
`_bmad-output/planning-artifacts/valuation-model-family-architecture.md` (**append `AD-VM-012`**; content enumerated in T5.9,
...
**L1, L2, L3, L4, L5, L6, L7, L8 and L9** hold and are tested — enumerated, not a range. **L9 was
outside the old `L1 through L8` range**, and L9 is constraint 6 closed by construction (`robust_mean`
takes no threshold; `MAX_ABSOLUTE_Z` is the workspace's only one). The ADR exists and contains every
required element of T5.9,
including the verbatim D5 statement and the register enumerated by id — LD-2, LD-3, LD-4, LD-5, LD-6, LD-7, LD-8, LD-9, LD-10 and LD-11 (LD-1 struck as closed by Wave 2) — **each with its detector**. Every completion statement
...
- **The latent-defect register** (D7), enumerated by id — LD-2, LD-3, LD-4, LD-5, LD-6, LD-7, LD-8, LD-9, LD-10 and LD-11 (LD-1 struck as closed by Wave 2) — each with id, owner, trigger **and detector**
...
- **Manual-procedure step:** a policy-fingerprint bump is now a triggering change. The step names
  all five fingerprint sites (**T2.9's** table — v3 cited T2.8, which is the defect-class sweep;
  Sensei r4 P0-6), says two of them are generated and must never be
...
| W4-P05 | positive | reader | the economic contract | it is read for the register | LD-2, LD-3, LD-4, LD-5, LD-6, LD-7, LD-8, LD-9, LD-10 and LD-11 (LD-1 struck as closed by Wave 2) each carry an id, an owner, a trigger **and a detector** | D7 |
...
| --- | --- | --- |
| Shell library | `cargo test --lib` | **518 passing, 3 failing, 22 ignored — 543 tests, committed tree at `4d1e916`.** This is the canonical baseline. See the box below: v5's "MEASURED 520" was the error, and the inherited **518** was right all along. |
...
> ### The baseline is resolved. Three different, all correct, numbers — and v5 recorded the wrong one as canonical.
>
> *(v6. Raised by Wave 3's builder, settled by orchestrator measurement, cause confirmed by Juan.)*
>
> | tree | reads | why |
> |---|---|---|
> | **committed `4d1e916`** — **canonical** | **518 / 3 / 22 — 543 tests** | what every wave is actually built from |
> | shared checkout | 520 / 3 / 22 — **545** tests | **+2 `#[test]` from Juan's own uncommitted work** in `engine.rs` (`upside_and_discount_use_distinct_denominators`, `nem_like_row_shows_upside_near_thirty_one_not_twenty_four`) — **unrelated to valuation** |
> | any builder worktree | 517 / **4** / 22 — 543 tests | committed tree, **minus** the parity test that needs untracked `random20-inputs.json` |
>
> **v5 asserted "MEASURED 2026-08-04: 520" and called the inherited 518 a discrepancy to be
> explained. It had it backwards** — 518 was right, and the "measurement" that overrode it was taken
> on a tree carrying two unrelated tests. §4.2 then wrote up the wrong number as a blocker.
>
> Evidence: `cargo test --lib -- --list` → **545** in the shared checkout; `git diff -- '*.rs'` adds
> exactly **+2 `#[test]`, 0 removed**; `#[test]` counts by commit are 543 at `4d1e916`, `3d01d5a`,
> `e4e152e`, `131e72b` — **545 never occurred on this branch.**
>
> **Juan's files are not to be touched, staged, or counted** (2026-08-04: *"son cambios que hice yo,
> tranqui, dejalos como estan"*). They are why constraint 7 exists.
>
> **Rule for every remaining wave:** baseline against **your own tree at your own start**, quote
> which of the three rows you are in, and diff against that. **The absolute number is not the
> control — the delta is.** Wave 3's 517 → 529 with an identical failing set is a clean pass under
> this rule and always was.
...
> **In a builder's worktree the set is FOUR, not three** *(v6; measured by Wave 3's builder,
> verified by the orchestrator)*. `cross_platform_parity::export_random20_sp500_parity_snapshot`
> also fails there, because it reads **untracked** `.agents/workspace/tmp/random20-inputs.json`
> (19,502 bytes, present in the main repo, absent from every worktree by construction).
>
> **This is an artifact of the worktree isolation the orchestrator owns — not a regression, and not
> attributable to any wave.** Builders baseline **four** and diff against their own start; what R-4
> protects is that the set does not *change state*, never that it equals three. **Nobody creates the
> missing file, adds a fixture to satisfy it, or relaxes the test** — all three would be gaining
> ground by weakening a check, and would mask a genuine failure of this test later. Every wave
> report states which baseline it used, so Stage 5 does not read the fourth name as a regression.

> **`high_signal_screener_cohort_all_members_pass` is NOT offline-deterministic — corrected in v6
> by Wave 1's measurement.** The plan's carry-list said it *"fails deterministically offline, not
> flakily online"*. **Network was available in Wave 1's worktree**, so it ran the live cohort and
> failed the gate at `pass=9/26` at start and `pass=9/26` at exit, same member list — but the
> underlying numbers moved between the two runs of the *same tree*: `PTC 16415 → 16480` (+0.40%),
> and OMC's market price `8148 → 8189`.
>
> **So its pass/fail state is a usable control; its per-issuer values are not.** No wave may quote a
> value from this test as a before/after comparison, and no wave may attribute a small anchor drift
> observed through it to its own change. The authoritative per-issuer before/after is the
> orchestrator's own paired run, taken close in time on one tree. Wave 1 checked exactly this before
> claiming no trigger fired: no anchor moved more than ±5% and none changed side of a gate.

Rules that apply to every wave without exception:

- **No wave may make a fifth test fail** (a fourth, when running outside a worktree — the rule is
  *no new failure beyond the baseline you recorded*, stated by name, never by count).
...
### 4.2 BLOCKER — the baseline tree is not stable, and one changed file is in Wave 2's gate

**Measured, not inferred.** `cargo test --lib` on the current tree returns **520 passed, 3 failed,
22 ignored**. The three failures are exactly the three protected names and the ignored count
matches. The pass count does not: the plan says 518.

The cause is established:

```
git diff --stat -- apps/windows/src-tauri/src/
 engine.rs            | 55 +++++++++-
 index_estimates.rs   |  1 +
 opportunity_v3.rs    |  3 +-
 quant_lens.rs        |  5 ++-
 regime/regime_fit.rs |  3 +-
 5 files changed, 60 insertions(+), 7 deletions(-)

git diff -U0 -- apps/windows/src-tauri/src/ | grep -c '^+.*#\[test\]'   →  2
```

**Two added `#[test]` in uncommitted work account for the difference exactly.** None of it is this
session's: the only source edit made here was F1's temporary `#[cfg(test)]` gate, reverted, with an
empty `git diff` on that file. These files were **not** in the session-start `git status` snapshot,
so the tree moved during the run.

Why this blocks Stage 4 rather than being a number to correct:

1. **Every wave's exit criteria are stated against this baseline.** "No wave may make a fourth test
   fail" and the pass count are how a wave proves it did no harm. Against a tree that moves under
   the builders, a count change has two possible causes and the builder cannot distinguish them.
2. **A builder that sees an unexplained count change will do one of two wrong things**: report a
   false regression, or investigate and "fix" code that was never broken. The second is worse and
   the plan's own rules do not protect against it, because they assume the baseline is fixed.
3. **`quant_lens.rs` and `engine.rs` are modified, and `cargo test --lib quant_lens::` is one of
   Wave 2's five mandatory gate commands.** Wave 2's gate would run against changed code of
   unrelated provenance, and its result would not mean what the plan says it means.
4. Parallel waves compound it: three builders on one tree already require the isolation discipline
   in §2.0, and that discipline assumes the *only* changes are the builders'.

**This is not a judgement about the uncommitted work** — the repo is documented as carrying
long-lived uncommitted work, and staging it is explicitly forbidden without instruction. The
requirement is only that the tree be **frozen and its baseline re-measured** before builders start.

**RESOLVED — Juan delegated the choice to the orchestrator (2026-08-04). Ruling: per-wave delta
baselines. The absolute count is retired.**

The files carrying the in-flight work — `engine.rs`, `index_estimates.rs`, `opportunity_v3.rs`,
`quant_lens.rs`, `regime/regime_fit.rs` — appear in **no wave's ownership matrix**. The waves own
`edgar.rs`, `dcf_model.rs`, `driver_resolution.rs`, `valuation_core_adapter.rs`, `numerics.rs`, the
contracts and the docs. So there is no file collision; the only exposure was the count and the
`quant_lens::` gate. Freezing the tree or moving to worktrees would buy determinism the waves do not
need, at a cost in either Juan's workflow or mandatory worktree cleanup.

Therefore, replacing the absolute rule everywhere it appears:

1. **Each wave measures its own baseline at wave start**, on its own tree, and records it verbatim.
2. **Every exit criterion compares against that wave's own start measurement**, never against a
   number written in this plan. The rule is *"this wave made no test fail that was passing when it
   began"* — which is what "no fourth failing test" was always trying to say.
3. **The protected set is named, never counted.** The three names are the invariant; `520` is not.
   A fourth *name* appearing is a stop; a pass count that rose between waves is Juan working, and is
   reported without investigation.
   - **Except inside a file this wave owns** *(v6, discharging carry item 18)*. There, constraint 6
     applies **first**: a count that rose because a test you touched now passes is not Juan working,
     it is the thing constraint 6 forbids until you have shown otherwise. State which files the wave
     wrote, and attribute every count change to a file inside or outside that set before invoking
     this rule. "Juan working" is the explanation of last resort, not of first convenience.
4. **`cargo test --lib quant_lens::` is judged as a delta**, before and after, for the same reason.

This is strictly more robust than freezing: it survives further drift instead of forbidding it, and
it removes a class of false alarm rather than promising nobody will trigger it. `520 / 22 / 3` is
retained below as the measurement of record for 2026-08-04, not as a gate.

### 4.1 Pre-flight, before Round 1 — orchestrator-owned

**T5.12 is promoted out of Wave 5 and runs here, before any wave starts.** This is a v3 correction
of an ordering defect: F1 (`valuation_core_adapter::value()` has no non-test caller) carries the
**live-QA exemption for Waves 3 and 5**, and Wave 3 runs in **Round 1** while T5.12 sat in Wave 5 in
**Round 3**. Two waves would have spent their exemption two rounds before the proof that entitles
them to it. Worse, if T5.12 had failed in R3, Wave 3 would already have shipped in R1 under an
exemption that was never valid — and the plan had no path to unwind that.

Pre-flight, in order, before Round 1 is dispatched:

1. **Record the cucumber baseline.** `cargo test -p valuation-core --test cucumber`. v1 omitted it
   and Wave 5 edits both feature files; the suite most likely to move had no starting count.
2. **Prove F1 by the compiler, not by grep.** Gate `value()` behind `#[cfg(test)]`, confirm the
   crate builds, revert the gate, record the result.
   - **If it builds, F1 holds** and Waves 3 and 5 keep their live-QA exemptions.
   - **If it does not build, F1 is false.** Both exemptions are void, Waves 3 and 5 acquire full
     live QA, and the round schedule is re-cut **before** any builder starts. This is a
     stop-and-replan, not a wave-level failure.
3. **Confirm the three protected failures are exactly the three named above**, by name, on a clean
   tree — so that "unchanged" means something at every wave's exit.

None of this is delegated to a builder: it is the evidence the wave assignments rest on, and a
builder proving its own entitlement to skip QA is the wrong shape.

#### Pre-flight RESULT — executed 2026-08-04 by the orchestrator

**1. Core baselines, recorded for the first time.** v1 omitted the cucumber baseline entirely, and
Wave 5 edits both feature files — the suite most likely to move had no starting count.

| Suite | Result |
| --- | --- |
| `cargo test -p valuation-core --test cucumber` | **6 features, 95 scenarios (95 passed), 629 steps (629 passed)** |
| `cargo test -p valuation-core --lib` | **89 passed, 0 failed, 0 ignored** |
| `cargo test -p valuation-core --test schema` | **7 passed** |

Wave 5's exit criteria compare against these numbers, by name. A scenario count that *falls* is a
deleted row and must be a replaced row (Decision 2); a step count that falls without a scenario
count falling is a weakened assertion.

**2. F1 holds, and the compiler proved something stronger than F1 claimed.** `value()` gated behind
`#[cfg(test)]`; `cargo check --lib` finished **exit 0**. The gate was reverted immediately and
`git diff` on the file is empty.

F1 asserted that `valuation_core_adapter::value()` has no non-test caller. The compiler agreed and
then went further: it emitted `never used` for the whole downstream subtree — `reading_variance`
(`:723`), `measured_or_absent` (`:734`), and the `IssuerEvidence` methods `discount_rate` (`:672`),
`net_debt` (`:696`), `diluted_shares` (`:706`) among 53 warnings. **The entire adapter is test-only
in production**, not merely its entry point.

Three consequences, all of which strengthen the plan rather than change it:

- **Waves 3 and 5 keep their live-QA exemptions**, now on compiler evidence rather than on a grep.
  The round schedule stands and no re-cut is needed.
- **Wave 3 cannot move a published number.** `fit_growth_path` and the naked means at `:280` and
  `:536` are inside the dead subtree. This is the expected state — `valuation-core`'s own module
  docs say *"Nothing here is wired into the running application yet"* — and it is why Wave 3 is a
  correctness fix landing ahead of the wiring, not a behaviour change.
- **The asymmetry with Wave 2 is now proven, not assumed.** Wave 2 touches `dcf_model.rs` and
  `driver_resolution.rs`, which are live; Waves 3 and 5 touch code the compiler says nothing calls.
  Wave 2 is the only wave in this run that can move a published valuation, and §6.2 says so.

**3. The protected failing set** is confirmed against a clean tree at the start of Round 1 by the
`cargo test --lib` run recorded in §4; the three names in §4 are exact.
...
| W5 | `_bmad-output/planning-artifacts/valuation-model-family-architecture.md` — **append `AD-VM-012`** | edit | W4 |
...
| W4 | **`docs/roic-target-specification.md` (T4.5)** | **new** | **W4** |
| W4 | `docs/index.md`, `AGENTS.md` | edit | W4 |

**T4.5 was missing from this table in v2** — it existed as a task (line ~2093) and in Wave 4's own
list, but not here, which is the list a builder reconciles against at the end. The target
specification is one of the brief's binding repository artifacts and the gate for LD-4's NCI-basis
question; omitting it from the consolidated list is how a required artifact quietly does not ship.
It carries all **seventeen** decisions the brief enumerates, and per Juan's pre-registration rule
any later change to the target or its exclusions is **a new experiment requiring a new untouched
holdout**.
...
> ### R1 EXITED — merged 2026-08-04 *(v6, R-14)*
>
> ```
...
> ```
> Branch `round1-integration`, worktree `G:/dev/repos/discount_screener-wt-round1`.
>
> **The independence argument held mechanically, not only semantically.** The two waves' file sets
> are disjoint — `edgar.rs` / `valuation_probes.rs` / `sec-point-in-time-provenance.md` against
> `valuation_core_adapter.rs` / `valuation-core/{lib,numerics}.rs` /
> `valuation-aggregation-audit.md` — so the octopus merge resolved nothing by hand.
>
...
> | ignored | 22 | **23** |
>
> `+29 = +17` (Wave 1) `+12` (Wave 3) — **each wave's own additions, with no residual** — and the
> failing set is the same four names. A residual would have meant the waves interacted; there is
> none.
>
> **The live baseline obligation below is discharged by construction.** W2a now takes its own
> "before" table on this merged base, so R2's before/after is post-Wave-1 by definition rather than
> by a separately captured artifact that could go stale between rounds.
>
> **W2a was dispatched against `4d1e916` and is re-pointed at `3bd20f2`.** That was an orchestrator
> defect against the binding row in Wave 2's own header; it was caught before any source edit, and
> the builder's draft T2.4 design — formed against the pre-Wave-1 shape — is discarded and
> re-derived rather than reconciled. *(R-14)*
...
- **Wave 2 changes published behaviour by design. It is the one wave that does.** v2 carried the
  sentence "Wave 2's is *proved* absent by T2.6" from v1, where it was true of the inert version of
  the wave. Juan's Q1 ruling removed the `.abs()` that made it inert, and T2.6 was replaced by
  sign-preservation tests in the same revision — but this sentence survived, and it is the single
  line that would let a reader merge Wave 2 without live QA. It is struck. **No wave may cite
  proved-absent behaviour change for Wave 2.** Waves 1, 3 and 5 intend no published-behaviour
  change: Wave 1's is measured against T1.7's count; Waves 3 and 5 rest on F1 and report anchors
  anyway.
- **Wave 2's expected effect is pre-registered in the wave**, before it runs: direction, magnitude
  and the affected-issuer set. A builder that observes an effect outside the pre-registered
  envelope stops and reports rather than rationalising it.
...
together. Rebuilding the growth engine. Re-capturing `core_driver_data_deep.json`. **LD-2, LD-3,
LD-5, LD-6, LD-7, LD-8, LD-9, LD-10 and LD-11**, each with its trigger condition and detector
recorded (D7).

**Android, in full — §0.0.** The hand-written Kotlin sign path (T2.5), the `KotlinList` empty-case
generator defect (the D6 T2.11), the Kotlin `abs()` twin of LD-1 at `DcfAnalysisEngine.kt:535-541`
and `:802`, every Kotlin test, and every Gradle run. The *generated* Kotlin is still emitted, because
narrowing the generator to one target would manufacture the drift §1 repairs. The divergence this
creates is **LD-10**; the fact that no instrument would catch it is **LD-11**.

Two corrections to v2's version of this list, both of which contradicted the register:

- **LD-1 is not out of scope. It is closed, by Wave 2, under Juan's Q1 ruling** — D7 already showed
  it struck through while this sentence still deferred it.
- **LD-4 is not out of scope either, and cannot be.** Its own detector in D7 is *"the T4.5 gate"*:
  the target specification cannot pin "invested capital" (rows 2 and 13) without resolving whether
  `stockholdersEquity` is NCI-inclusive or NCI-exclusive. **T4.5 is in this run.** So the NCI-basis
  question is forced open by work this plan schedules, and listing it as deferred would let a Wave 4
  author write around it. What is out of scope is *changing* the equivalence class — that moves
  invested capital for every issuer with a material minority. **T4.5 must resolve and record the
  basis; no wave may alter it.** The distinction is the whole point: decide it in the contract, ship
  it in a later run. Any change to `MAX_ABSOLUTE_Z`, to a test threshold, or to a refusal path that would gain
...
- **(b′)** **an anchor stops producing a valuation at all — unconditionally, with no exemption.**
- **(c)** a choice between fixing something and refusing to value it.

**(b′) is new in v3 and it exists because v2's trigger (b) could be evaded by construction.** v2
exempted movements arising *"other than through T2.7's declared refusal"* and did not carve the
anchors back out. Both r3 reviewers reached the same exposure independently, and it is confirmed
here against the code rather than argued:

- `edgar.rs:1096-1101` is the sole production call site of `with_rate_resolution_inputs` and passes
  `None, None` for the market-yield and rated-spread rungs. **Verified — the literal `None, None`
  is at `:1099-1100`.**
- With the accounting rung emptied there is no other rung, so control reaches the terminal `else`
  at `driver_resolution.rs:236-239`, which returns `Err("fcff unavailable: no aligned market yield,
  spread, or SEC interest/debt periods")`. **Verified — read at `:235-240`.**
- That `Err` propagates by `?` through `derive_wacc` and `fcff_wacc`.

So **T2.7's "refuse the cost-of-debt channel" is, in production, "refuse the entire FCFF
valuation."** A refusal that is correct at the channel is a blackout at the issuer. Under (b) as v2
wrote it, an anchor could go dark and no builder would be required to stop. Under (b′) it must.

An anchor going unavailable is *stronger* than moving 5 percent, not weaker, and it is exactly the
case Juan's standing instruction covers: **"no seas vos el que decide si 'vale la pena' alejarse de
un anchor."** A builder does not get to classify a blackout as a declared refusal and continue.

**Pre-registered affected set, recorded before Wave 2 runs — corrected by T2.0's measurement**
*(R-7.1)*. Through v5 this section registered **COF (19 years), DAL (15), CHTR (12), BKR (11)** on
the basis of *how many years each files `InterestIncomeExpenseNet`*. That is **extraction
incidence**. The quantity a pre-registration exists to protect is **published effect**, and T2.0
measured the two to be **disjoint**: the four registered names move **$0.00 published between
them**, and the single name that does move was named nowhere in six plan revisions.

The plan therefore carries **two separate sets**, and no reader may substitute one for the other:

| set | members | what it is for |
|---|---|---|
| **extraction incidence** | COF (19 yrs), DAL (15), CHTR (12), BKR (11) | which issuers resolve the net concept at all. Retained — it is true, and it is what **LD-8** keys on |
| **driver-level affected, S&P 500** | **24 distinct issuers / 25 symbols, 5.0% of 496 scanned** (list below) | whose **interest series** changes. Δmargin 0–271 bps |
| **published effect** *(THE pre-registration)* | **6 issuers: ROST, MPWR, JKHY, ULTA, CPRT, NKE** | whose **published number** actually moves. Measured, not inferred |
| **published effect, gate cohort** | **`{MPWR}`** | what the 26-name gate can observe |

None is an anchor. This is the plan's prediction, not its permission: **any issuer outside `{MPWR}`
with a non-zero published delta fires the trigger**, as does MPWR landing outside its measured band
or moving *upward*, as does any change to an anchor at all. Recording the set is what makes the
surprise detectable; it is not a reason to skip checking.

**The wider scan returned, and `{MPWR}` alone was wrong** *(v6, R-10)*. The archetype is **common**:
**496 issuers scanned** (universe 501, 5 without a CIK), **25 symbols / 24 distinct issuers are
`OperatingNonFinancial` with ≥1 net-interest year filed positive — 5.0%**:
...
Max |Δmargin| distribution, bps of revenue: `n=25, min=0, median=53, max=271` —
`0 2 7 7 12 14 14 22 24 29 29 38 53 67 88 109 112 112 114 123 133 170 209 224 271`.
**Ten of twenty-five move ≥100 bps. MPWR is not the largest — CPRT moves further.**

Also measured: **107/496 (21.6%)** file a net-basis concept at all; **48 (9.7%)** file it positive;
and **22 of those 48 are `FinancialServices`** — enormous counterfactual `dFCFF` (COF $78.5bn,
C $87.2bn) reaching **$0.00 published**, exactly the COF result generalised.

### The published-value counterfactual — THE pre-registration *(v6, R-13; measured before W2b runs)*

Measured by deleting exactly LD-1's three `.abs()` sites in an isolated worktree and applying the
negation in the probe's arithmetic. **That worktree must never be merged**; the diff is retained at
`build/r10-counterfactual.diff` for comparison against what W2b actually produces.

> **Of 25 driver-level affected issuers, 6 move a published number — all downward.**
> `n=6, min −157 bps, median −60 bps, max −18 bps` of the before value.
> Sorted: `−157 −101 −82 −39 −22 −18`.
> In cents: **ROST −279, MPWR −357, JKHY −135, ULTA −124, CPRT −23, NKE −12.**
>
> **19 of 25 move $0.00. 0 change router lane. 0 produce a non-positive FCFF. 4 anchors move $0.00.**

**Anchors, checked first:** `PG 18109→18109`, `GOOGL 35679→35679`, `AMZN 16185→16185`,
`MSFT 57139→57139` — and all four at `yrs=0`, meaning **not one fiscal year differs** between the
two histories. Structural, not a near miss. **Trigger (b) does not fire.**

**Zero router-selection flips anywhere** — the qualitative effect that would have outranked any
magnitude. Every issuer publishes the same lane before and after, MPWR included (`disp:fwd` both
sides). The nearest approach moves *away* from a flip: MPWR's gap was 5062 bps against a 5000
threshold and the correction drops its FCFF candidate `54630 → 51585`, **widening** it.

**Why 19 absorb it, and why that is not the change failing to arrive.** The tree check confirms all
25 have rewritten years. The correction is absorbed by **robust normalization over the series** —
WSM's interest add-back flips outright, `28 → −28` bps, and its published value does not move a cent
— and by the router publishing the **forward** lane. This is Wave 3's robust aggregation doing
exactly what it exists to do, observed on live data.

**Both a prediction that all 25 move and a prediction that none do would be falsified by the wave.**
The registered claim is the six names, the direction, and the distribution above.

**No refusal row to pre-register:** no corrected FCFF goes negative or non-positive. CARR, DDOG,
NWS, NWSA and TTD have no FCFF candidate on *either* side — pre-existing absence, not a refusal this
change causes.

**Both rows above are true of different populations, and conflating them is the R-8.2 error.**
Inside the 26-name gate cohort the mover really is MPWR alone — the intersection of the 25 with
`{DVN FIS AVY SW COF MPWR APH EME CHTR BKR INTU TER AVGO EPAM T GEHC DAL WDC GOOGL GOOG HPE CRM SLB
EXE OMC PTC}` is exactly `{MPWR}`. **No anchor is in the affected set.** So the gate observes one
name while the product moves twenty-four, and **no report may quote the gate figure as the blast
radius.**
...
## 7. Changelog, v5 to v6

**One cause: T2.0 executed, and its measurement overturned the pre-registration.** No reviewer
round produced this revision — the blocking pre-flight the plan itself ordered did. Full reasoning
in `plan-review/ORCHESTRATOR-RULINGS.md` **R-7**; evidence in
`build/t2.0-filed-sign-measurement.md`.

1. **§6.2's pre-registered set was measuring the wrong quantity.** It registered **extraction
   incidence** (how many years each issuer files the net concept: COF 19, DAL 15, CHTR 12, BKR 11)
   while claiming to register **published effect**. T2.0 measured the two to be **disjoint** — the
   four registered names move **$0.00 published between them**. Split into two explicitly separated
   sets; the published-effect set is **`{MPWR}`**. *(R-7.1)*
2. **MPWR added to the plan, at all.** A member of the 26-name high-signal gate cohort
   (`valuation_high_signal.rs:669`) whose published FCFF margin moves **−77 to −224 bps every net
   year**, named in **none** of the six prior revisions.
3. **Pause trigger (c) split into (c.1) refusal / (c.2) published effect.** (c) keyed entirely on
   the cost-of-debt guard firing; MPWR has **no filed debt**, so its guard never fires while its
   published number moves. **The wave's only real effect was invisible to its own trigger.**
   (c.2) needs no invented threshold — the prediction for every name but MPWR is exactly zero.
   *(R-7.2)*
4. **A magnitude band, in both directions.** MPWR landing outside **−77 to −224 bps**, or moving
   *upward*, is now itself a stop — so the registered name cannot silently absorb an arbitrary
   result.
5. **The direction is recorded as correct, in advance.** `OCF` already contains interest received;
   `.abs()` made that income look like an expense and the bridge added it back, **double-counting
   for twelve years**. The drop is a correction of an overstatement, not a regression — written
   before the number moves so no Stage 5 reviewer reads it as damage and no one proposes recovering
   the 224 bps. *(R-7.3)*
6. **The claim's scope is stated honestly.** `{MPWR}` is complete **within 29 names**, not within
   the model. The archetype — net-cash, debt-free, filing net interest income — is common among
   semis and software and unmeasured outside the cohort. A wider scan is ordered; until it returns
   the section claims **cohort-local completeness only**.
7. **T2.0 marked EXECUTED** with all four discharged items, the BAC `+60,096M` control that proves
   the zeroes are a result rather than a dead instrument, and the two follow-up measurements still
   open.
8. **R2's risk row** rewritten to name the correct control.
9. **T2.12's planted fixture row switched from synthetic to real** — MPWR's filed series
   (2014 `+1.1M` … 2025 `+29.2M`) discriminates the same convention while being falsifiable against
   EDGAR, which an invented value never is. Written as synthetic only because no measured
   net-income series existed when the task was drafted.
10. **T1.7's PIT-coverage sample gains MPWR** (`probe_facts_without_a_filing_date`). It named the
    four Wave 2 issuers, all of which move $0.00 — a sample that omits the only issuer the wave
    moves cannot observe the wave it samples. **T1.7 sits in Wave 1, which was already dispatched
    against v5**, so this was sent to the running builder as an amendment rather than left for it
    to discover in a file it will never re-read.
### Wave 3's delivery, folded in (R-8)

12. **W3-E03 was impossible as specified** — at n=5 the keep-threshold is `≈4.45·d₃`, so three
    observations always survive, and the zero-dispersion edge refuses rather than retaining two.
    The plan **contradicted its own K5**, which already called a retained count of two unreachable.
    The builder implemented the reachable form plus an assertion of the impossibility — **stricter
    than specified**. Accepted. *(R-8.1)*
13. **The pinned cohort is 20 names, not 28** (two sites corrected), and — more consequential than
    the count — **it contains no MPWR and none of COF/DAL/CHTR/BKR**. Wave 3's zero refusal-rate
    change therefore says **nothing** about the issuers Wave 2 moves. The 26-name gate cohort and
    the 20-name pinned cohort are now distinguished at the point of use. *(R-8.2)*
14. **§4's protected set is FOUR in a worktree, not three.**
    `cross_platform_parity::export_random20_sp500_parity_snapshot` reads untracked
    `random20-inputs.json`, absent from worktrees by construction — **an artifact of the
    orchestrator's isolation, not any wave's regression.** Nobody creates the file or relaxes the
    test. Sent to both running builders. *(R-8.3)*
15. **A fourth clause added to the verification rule: red must be *isolated*.** A combined mutation
    that turns everything red is the evidence shape that hides a vacuously-passing test — Wave 3
    caught exactly that in one of its own pins. *(R-8.4)*
16. **§4's Shell baseline corrected from 520 to 518, and §4.2's blocker dissolved** — v5 recorded
    *"MEASURED 520"* and treated the inherited **518** as the discrepancy. **Backwards.** 518 is
    right; the 520 reading was taken on the shared checkout, which carries **+2 `#[test]` from
    Juan's own uncommitted work in `engine.rs`**, unrelated to valuation and not to be touched.
    Three trees now recorded with the canonical one named, and the rule restated: **the control is
    the delta against your own tree, not the absolute count.** *(R-9)*
### The wider scan and the gate check returned (R-10)

17. **`{MPWR}` was wrong — the affected set is 24 issuers / 25 symbols, 5.0% of 496 scanned.** Ten
    move ≥100 bps, max **271 bps**, and **MPWR is not the largest**. My R-7.1 registration was
    cohort-local; I flagged it as such and the scan justified the flag. §6.2 and trigger **(c.2)**
    re-keyed to the measured set — `{MPWR}` alone would have fired on twenty-four true positives.
    *(R-10.1)*
18. **Trigger (b) does NOT fire.** MPWR fails its gate today, its published base is the **forward**
    candidate (`91657c`) not FCFF (`54630c`), the counterfactual is identical on every field, and it
    **cannot** cross in this direction by arithmetic. Recorded as a **measured negative**, not as
    absence of evidence. Wave 2b proceeds. *(R-10.2)*
19. **W2a and W2b are two review units and ONE merge unit — binding.** W2a's inertness was
    *measured* (`Some(85)` → `Some(85)` bps after rewriting twelve years of interest; both read
    sites re-`.abs()` at `:551` and `:1590`). W2a alone would ship a contract and a `/9` fingerprint
    **declaring a convention the code does not honour**. Its inertness is what makes it unsafe to
    ship alone. *(R-10.3)*
20. **The gate/product asymmetry is now stated where it can be misread.** Inside the 26-name gate
    cohort the only mover is MPWR; across the S&P 500 it is twenty-four. **No report may quote the
    gate figure as the blast radius** — that would be R-8.2's cohort conflation committed twice.
21. **Still carried:** the pre-registration now states magnitude at **driver level** (Δmargin bps
   across 25 names) but **not** at **published intrinsic-value** level. The enabling counterfactual
   is a two-line production edit, ordered as an isolated measurement so W2b lands against a number
   registered *before* it rather than reported after it. **No threshold moves, no ticker
   special-case** — if an issuer leaves the cohort because its FCFF was overstated, the cohort is
   wrong, not the correction. *(R-7.5)*

### Wave 1's delivery, folded in (R-11)

22. **T1.7 measured, and the fail-close costs this sample nothing.** 8504 accepted 10-K facts across
    17 issuers (four anchors + the Wave 2 issuers **including MPWR** + an oldest-history slice of the
    gate cohort): **0 missing `filed`, 0 unparseable `end`, 0 missing `accn`** — per issuer, in every
    column. The concern that requiring all three would silently thin the series is answered on this
    sample by measurement rather than by argument. Written into
    `docs/sec-point-in-time-provenance.md` as a *measurement on 17 issuers*, explicitly not a proof
    for the universe. *(R-11)*
23. **305 disagreeing period-ends — the number that justifies retaining vintages at all.** Live SEC
    data really does file the same `(concept, period_end)` twice at different values, so `as_of` and
    `latest` **do** disagree on real data. **A zero here would have meant Wave 1 built a mechanism
    nothing can distinguish from `latest`** — the check that would have passed while proving nothing.
    CHTR alone reads 0.
24. **Wave 1's own instrument defect, found by the builder.** W1-N02's mutation M3 survived because
    the period-shape check rejected the input *first* — the assertion never reached the code it was
    written to pin. Sixth instance of the "check that cannot fail" pattern this session, and the
    second found by a builder auditing its own work rather than by a reviewer. Feeds v6's fourth
    verification clause. *(R-11)*
25. **The worktree was 28 commits stale, and the baseline is 517/4/22, not 520/3/22.** The builder
    reset to `4d1e916` before touching anything and measured its own start, per R-4. The fourth
    failing name is the orchestrator's isolation artifact
    (`cross_platform_parity::export_random20_sp500_parity_snapshot`), not a regression. Exit: same
    four names, `+17` passing, `+1` ignored — all this wave's own additions.

### The published-value counterfactual, folded in (R-13)

26. **`(c.2)` re-keyed a second time: 25 symbols → the 6 issuers that actually move a published
    number.** `{ROST, MPWR, JKHY, ULTA, CPRT, NKE}`, all downward,
    `n=6, min −157, median −60, max −18 bps` — in cents `−279, −357, −135, −124, −23, −12`. The
    25-symbol registration would have fired on **nineteen true negatives**. This is the third
    revision of the registered set (`{MPWR}` → 25 → 6) and **every one was driven by a
    measurement, not by an argument**. *(R-13.1)*
27. **19 of 25 move $0.00 — and not because the correction failed to arrive.** The tree check
    confirms all 25 have rewritten years; WSM's interest add-back flips outright (`28 → −28` bps)
    and its published value does not move a cent. The correction is absorbed by **robust
    normalization over the series and by the router publishing the forward lane**. That is Wave 3's
    aggregation doing what it exists to do, observed on live data. *(R-13.2)*
28. **Zero router-selection flips, anywhere — registered as the qualitative stop.** A lane flip
    would outrank any magnitude. None occurred, MPWR included (`disp:fwd` on both sides), and the
    nearest approach moves *away*: MPWR's `54630 → 51585` **widens** its 5062-vs-5000 bps gap.
    *(R-13.3)*
29. **All four anchors move $0.00 at `yrs=0` — structural, not a near miss.** Not one fiscal year
    differs between the two histories for PG, GOOGL, AMZN or MSFT. **Trigger (b) does not fire**,
    and any anchor movement in W2b now means the wave did something the deletion did not. *(R-13.4)*
30. **No refusal row to pre-register.** No corrected FCFF goes non-positive. CARR, DDOG, NWS, NWSA
    and TTD have no candidate on *either* side — pre-existing absence, not a refusal this change
    causes. Registering it as empty is what makes a single new refusal a stop. *(R-13.5)*
31. **The counterfactual's own instrument defect, found mid-measurement.** A `BLOCKED` label would
    have reported "the counterfactual isn't working" when it was working and being absorbed —
    inverting the conclusion. Seventh instance of the pattern; third found by a builder on its own
    work. **The worktree carrying the three `.abs()` deletions is marked
    `T2.0/R-10 COUNTERFACTUAL — MUST NOT MERGE`**; the diff is retained at
    `build/r10-counterfactual.diff` as the comparison target for what W2b actually produces.
    *(R-13.6)*

### Round 1 merged, and W2a's base corrected (R-14)

32. **Round 1 is merged: `3bd20f2`, 546/4/23, `+29` reconciling exactly to `+17` Wave 1 and `+12`
    Wave 3.** File-disjoint, no manual conflict resolution, same four failing names. §6.1 now
    carries the exit box. *(R-14)*
33. **W2a was dispatched against the wrong base — mine, not the builder's.** Wave 2's header says
    *"Round 1 must be merged"* and §6.1 names the failure exactly: writing the sign into the
    pre-Wave-1 shape of `extract_annual_any_with_shape` **silently drops either the sign or the
    vintage retention, and both are invisible in a green build.** Caught before any source edit; the
    builder's draft T2.4 design is discarded and re-derived, not reconciled. Two questions added to
    its brief that §6.1 implies but does not spell out — whether the sign travels with the fact
    *through* resolution (a gap-filled year keeps the **filling** concept's provenance, so a
    post-merge scalar sign mis-signs it), and whether a sign change can flip a per-concept
    material-restatement verdict. *(R-14)*
34. **Wave 5 is not promoted into Round 2, and the check was made rather than assumed.** §6.1 allows
    promotion only *if Q1 blocks Wave 2 indefinitely*; Q1 is answered. The stated cost stands:
    concurrent Wave 5 would put a coverage collapse and a sign correction into one report. *(R-14)*
35. **Eight defects this run, and this one is a different species.** Seven were *"the check measures
    something other than what it protects."* This one is *"the dependency was written down, in the
    right place, in binding language, and the orchestrator acted without reading it."* It was found
    while checking whether Wave 5 could run in parallel — **the check that caught it was looking for
    something else.** Both belong in the retro; the remedy for the second is that the row saying
    what a wave stands on is not optional reading, and a builder is the wrong place to discover it.
36. **W2b built clean and STOPPED, and the reason was my defect.** R-13 was measured on a worktree
    that never touched `driver_resolution.rs` — it measured **T2.6 alone** while W2b is T2.6 **+**
    T2.7. The registration was not falsified by the wave; **it was never a prediction of this wave.**
    The attribution proof is MPWR: the only issuer T2.7's refusal cannot reach (no filed debt) is the
    only one that reproduced its registered move to the cent. Instance eleven, committed by me, in
    the artifact whose purpose was to prevent it. *(R-17)*
37. **The guard rule was re-measured on Juan's ruling rather than argued.** Two rounds: basis
    classification (Probe H, 52 symbols, 772 issuer-year rows) and published value (three arms, one
    fetch). Fifteen predictions registered across both, **five falsified**. *(R-18, R-20, R-21, R-23)*
38. **My own recommendation was measured as the worst option and withdrawn.** Reading the contract
    showed W2a negates the net concepts at extraction, so a negative value means *net basis AND
    income exceeded expense* — making (C) **strictly wider** than (A), not narrower. I had described
    its extra refusals as a benefit without noticing "reaches DAL/CHTR/BKR" means "takes them dark."
    Measured cost: (C) 26 channels of 48, (A) 23, **(D) 8**. *(R-18.2, R-20.3)*
39. **(D) came from a structural fact, not from the counts.** The six gross concepts precede the two
    net ones and `merge` fills by `.or_insert`, so a net concept wins a year **only when every gross
    concept is absent for it**. That admits refusing the *year* on basis instead of the *issuer* on
    sign. Marked post-hoc in construction, then scored on criteria fixed before the numbers existed.
    *(R-18.3, R-18.5)*
40. **The sign was sound but incomplete; its defect was the response, not the detection.** `FP = 0`
    across 772 rows — it is never wrong when it fires. But **every one of the 25 trigger years is
    `fittable? = no` by construction**: the year that vetoes an entire issuer is never a year the fit
    would consume. *(R-20.2)*
41. **(D) ruled the contract, with a bit-level pre-registration.** Cascade eliminated — TYL, YUM,
    ABBV and ROL return to base **exactly**; flips 9 → 3; movers 18 → 12; anchors `$0.00`. LD-8
    closes where the basis reaches a published number. **Arm D reproduces R-13's counterfactual
    bit-for-bit on four of six movers** — the registration I mis-filed was predicting (D) all along,
    which sharpens R-17.1 rather than excusing it. *(R-23, R-24)*
42. **No published-value regression gate exists anywhere in the repository.** The high-signal gate
    asserts a *classification*; `valuation_baseline.rs` asserts *absurdity*; the observation audit is
    a dump no assertion reads. W2b moved 18 published values across `−2897..+4988` bps with a fully
    green suite. **This is the structural reason every wave in this run has needed a hand-written
    pre-registration**, and it is the most durable finding of the effort. *(R-22)*
43. **The fixture capture forbids fabrication in a comment and commits it three lines below.**
    `effective_tax_bps.unwrap_or(0)` and `marginal_tax_bps.unwrap_or(2_100)` write the US statutory
    rate into committed evidence; a fabricated marginal tax turns an issuer that should refuse into
    one that fits. Routed with its reader/writer break. *(R-19)*

---

## 8. Changelog, v4 to v5

Round 5 — the last before the Stage 3 cap — returned `revise` from both reviewers: Sensei 8
build-blocking / 6 carry, Advisor 3 build-blocking / 2 carry. Every blocker is closed below. The
Sensei's summary of v4 was **"v4 fixes claims and does not fix instructions"**, and it was correct:
most blockers were sections that argued the right thing while the text a builder executes still said
the old thing.

**The one substantive blocker, and the measurement that closes it**

1. **T2.0 added, blocking W2a: measure the filed sign before building.** Two incompatible traces were
   argued this session — mine (COF positive, the other three negative) and the Sensei's (DAL/CHTR/BKR
   **bit-identical** before and after) — resting on the *same unmeasured premise*. `brief.md:30-32`
   describes the value **after** `.abs()` and says nothing about the filed sign, and it cannot be
   recovered from fixtures: every valuation fixture holds **zero** negative interest values
   *[executed 2026-08-04]*, because capture happens post-`.abs()`. **The evidence was destroyed by the
   defect this wave removes.** If the Sensei's trace holds, W2b's predicted published change is
   *nothing, anywhere*, and the affected class — `OperatingNonFinancial` **and** filing net interest
   income — is named nowhere in this plan. T2.0 settles it and discharges four things at once: the
   affected set, LD-8's exposed set (and Juan's binding condition with it), pause trigger (c)'s class,
   and whether W2b moves any number at all.
2. **The DAL/CHTR/BKR ruling is re-grounded.** v4 argued "refusing them would discard coverage,"
   which invokes Juan's closing constraint needlessly. The stronger ground: **this wave does not change
   them**, so refusing them would be a *new* refusal of currently-published issuers over a pre-existing
   defect — scope expansion, not Decision 1. Both statements are now marked **conditional on T2.0**.
3. **Pause trigger (c) re-keyed by class.** It was set on the flat set {COF, DAL, CHTR, BKR}, which
   tolerates the observation that would falsify the ruling and fires on none of the predicted ones.
   Now: any guard firing other than COF is a stop; any non-zero delta for the other three is a stop;
   the net-income-operating set is unmeasured until T2.0.
4. **LD-8's "no mechanical detector" downgraded to an overclaim.** The two-run extraction diff yields
   the exposed set today with no `FcfPoint` change. LD-8 carries the detector or states that it was
   priced and declined.

**Instructions that still ordered the old behaviour**

5. **W2a had no exit criteria, no baseline, no measurement** — its inertness was asserted, not
   observed, which is the shape of the invariance proof Juan's Q1 ruling already killed. W2a now has
   its own Done-when, including **J6 must be FALSE at W2a's exit**, a live all-zero per-issuer table,
   and a pre-condition search for extractor-local consumers the setter-based argument cannot cover.
6. **T2.14 added — the alignment check D6 mandates and no task delivered.** Reconstruct the sign array
   from `negatedQnames` and compare element-wise. A right-length, wrong-value array — a transposition
   of the two negated indices — passes J1 and passes W2-P03's count and inverts the economics.
7. **Four live instructions still ordered Android work §0.0 cuts**, one of them inside a *binding*
   D-decision. All struck.
8. **Blind grep, third revision running.** `interest.*abs()` misses the setter at `:907`; it survived
   in T2.6's acceptance, in Evidence of pass and in §0's own evidence line. All three now reference the
   canonical J6 check by name; no pattern is retyped anywhere.
9. **Gate count said "four" over a five-row table** in three places, and the likeliest drop was
   `valuation_gap_attribution::` — the fifth, added in v3 *because it was missing*.
10. **LD ranges: fixed in one of four sites.** `LD-1 through LD-7` still live at three, and both Wave 4
    lines dropped **"and detector"** while D7's own rule is *"a trigger nobody checks is a comment."*
    Also Wave 1's `I1 through I7`. All enumerated by id.
11. **Duplicate ids:** T2.11 was still two tasks *directly below a claim that the duplicate was fixed*
    (documentation → **T2.13**); **W2-P06 appeared twice** (new in v4 — the LIN bridge row → **W2-P07**);
    J10's body still called itself J7.

**The ADR relocation was half-done — my fix, asserted as complete**

12. **Three stale `docs/adr-0001-…` references survived** at §2.0's ownership matrix, Wave 5's task list
    and §5's deliverables table, while the v4 changelog claimed the relocation was done. A builder
    consulting either table would have created the second ADR series T5.9 exists to prevent. Fixed, plus
    ownership rows added for `valuation-model-family-architecture.md` and `docs/valuation-economic-contract.md`.
13. **The register's home was made to look conditional.** Binding D7 says unconditionally that the
    register lives in `docs/valuation-economic-contract.md` and the ADR links it; v4's T5.9 said the ADR
    *contains* it. D7 wins. The heading-capacity question is moot — AD-VM-002 and AD-VM-011 already carry
    multi-row tables. The real reason is that the register is **living** and does not belong inside a
    point-in-time decision record.
14. **The D7 register's own markdown was broken exactly where I extended it** — blank lines at the rows
    I added terminated table continuity, so LD-9, LD-10 and LD-11 would not have rendered as rows, and
    LD-9 sat after LD-11. Rebuilt as one continuous, ordered 11-row table.

**Carried as recorded risk, not blocking**

15. The read-side sweep reports 15 sites across 6 files while §0's blast-radius table rules on 10 across
    5 — five sites and one file unruled. **Promote to a W2a pre-condition**: paste the sweep and rule each row.
16. §6.2 claims a pre-registered *magnitude* that appears nowhere. It is derivable — for a net-income
    filer the bridge adds `+|interest|` after tax where it should subtract, so FCFF falls by roughly
    **2 × interest × (1 − tax)**. T2.0 measures it; write it or strike the word.
17. **The `[executed]` ledger is applied to 7 of ~40 checks and its "inherited" side has zero real
    entries** — the only hit is the rule statement itself. A ledger with no negative entries confers a
    verified appearance on the untagged remainder. Either tag every check or say plainly which waves'
    checks are inherited unexecuted.
18. `high_signal_screener_cohort_all_members_pass` fails deterministically offline, not flakily online;
    say so in §4. R-4's "a rising count is Juan working" needs one clause excepting wave-owned files,
    where constraint 6 applies first.

**Disposition of items 15–18 in v6** *(each was carried across three revisions; carrying them a
fourth time would be how a recorded risk becomes a forgotten one)*

- **15 — still open, and now a hard W2a pre-condition.** The 15-vs-10 discrepancy is not resolved by
  argument; W2a pastes the sweep and rules each of the fifteen rows, including the unruled file.
  A W2a report that does not contain fifteen ruled rows is incomplete. *(unchanged, promoted)*
- **16 — DISCHARGED by T2.0.** The magnitude is no longer derivable-in-principle, it is measured, at
  two levels and with the two explicitly distinguished: driver-level Δmargin `0–271 bps` across 25
  symbols, and published effect `n=6, min −157, median −60, max −18 bps`. §6.2 carries both; the
  word "magnitude" now has a number behind it in each row. *(R-10.1, R-13.1)*
- **17 — still open, and honestly stated rather than closed.** The `[executed]` ledger covers **7 of
  ~40** checks and its inherited side still has no real negative entries. Rather than tag forty
  checks at the end of Stage 4 — which would produce forty *claims*, not forty executions, and is
  exactly the defect the ledger exists to prevent — **the remainder is declared untagged and
  therefore unexecuted.** Any check not carrying `[executed <date>]` is inherited and unverified.
  That is the plainest form of the second option the item offered.
- **18 — DISCHARGED, and the premise it rested on was wrong.** The test is not offline-deterministic;
  Wave 1 ran it with network available and its values moved between two runs of the same tree.
  §4 now states the corrected form and the rule it implies (state is a control, values are not). The
  R-4 clause is written at §4's rule 3. *(R-11)*

## 9. Changelog, v3 to v4

Round 4 returned `revise` from both reviewers (Sensei 7 P0 / 5 passes; Advisor 2 P0 / 4 passes), and
Juan issued a scope ruling mid-round. Every item below was verified against the working tree before
being written; the orchestrator's own decisions and their evidence are in
`plan-review/ORCHESTRATOR-RULINGS.md` (R-1 … R-6).

**Scope**

1. **§0.0 added — Android is out of scope, Windows only** (Juan, verbatim). Cut: T2.5, the D6 T2.11,
   W2-P04, W2-P05, the Gradle run, and the four Android rows of the file-ownership matrix. **Not**
   cut: the contract, the fingerprint bump, and the generator's emission of *both* targets — see 2.
2. **The generator is not narrowed to Rust.** Emitting one target would manufacture precisely the
   generator/output drift §1 exists to repair. Generated Kotlin gains `qnameSigns` inertly;
   compile-safety verified three ways (declaration is self-contained at
   `SecDriverNormalizationPolicyGenerated.kt:4`; its one consumer reads by named field at
   `SecDriverNormalizationPolicy.kt:72-81`; nothing destructures or `copy()`s it).

**Corrections to claims v3 asserted and did not check**

3. **The COF premise was false, and it had reached Juan.** v3 (and the orchestrator's escalation)
   held that refusing COF's interest channel takes down its FCFF valuation. COF is
   `FinancialServices`, dispatched to `residual_income` at `dcf_model.rs:1263-1265` — `fcf_history`
   is not even a parameter. `derive_wacc`'s only callers are `fcff_wacc` (`:2350`) and
   `resolve_attribution_wacc`, whose sole caller is a self-disclaiming diagnostic
   (`valuation_gap_attribution.rs:1722`, `:32`, `:146`, `:1325`) whose `Err` arm writes a
   `source_note` and continues (`:1742-1744`). COF's e2e gate asserts `residual_income_equity` and
   is untouched (`cof-detail.native.e2e.mjs:83,88`). **LD-8's exposed set is three — DAL, CHTR,
   BKR — not four.**
4. **`cross_platform_parity.rs` is an exporter, not a check** (**LD-11**). Both its `#[test]`s assert
   only `path.is_file()` (`:319`) and `members.len() == 20` (`:513`) — no value assertion — and write
   to an untracked path (`:123-133`). The comparator that would check them,
   `scripts/compare-windows-android-valuation-parity.ps1`, is invoked by nothing but its own
   self-test. **R-1's original rationale cited this file as a working instrument; that was false and
   is struck.** The W2a/W2b split stands on reviewability instead.
5. **F16b added.** v3 read F16 as "this repo has no ADR" and invented a second decision series.
   `valuation-model-family-architecture.md` carries `AD-VM-001` … `AD-VM-011` and `AGENTS.md:590`
   already labels it *"valuation ADRs"*. T5.9 now appends **`AD-VM-012`** instead of creating
   `docs/adr-0001-…` and a new `docs/index.md` section.
6. **§1.6's Kotlin row was self-contradictory** — it granted the silent default that T2.5 forbade two
   sections later. Struck rather than quietly corrected (Sensei r4 P0-6).
7. **T4.7 cited T2.8 for the five fingerprint sites**; the fingerprint table is **T2.9**. Fixed.
8. **Ranges enumerated by id.** T5.9 said "LD-1 through LD-7" while the register already ran to LD-9.
   A range that silently excludes the newest entries lets a wave be declared done without them.

**Structure and rulings**

9. **Wave 2 splits into W2a and W2b** (R-1). W2a — contract, generator, both generated targets, Rust
   extraction sign, fingerprints, T2.12 — with the three `.abs()` sites still standing, therefore
   provably inert and reviewable against a fixed output. W2b — T2.6, T2.7, T2.8, T2.10, T2.11 —
   moves numbers deliberately, against a base proven not to have moved them by accident.
10. **T2.7's ruling row corrected** (Sensei r4 P0-2). "Fall through to the other cost-of-debt path"
    was false: `edgar.rs:1099-1100` passes `None, None` and `driver_resolution.rs:236-239` returns a
    terminal `Err`. **Channel refusal is issuer blackout.** Verified that *no producer exists
    anywhere* for either alternative rung — all 12 mentions are a declaration, a `None` default, a
    setter and two consumers — so building the missing rung would require a new market-data
    integration or a synthesized spread, i.e. *selecting an estimator*. Foreclosed, not declined.
11. **The four-issuer question is ruled, split by class** (R-5, routed to the Advisor by Juan). COF:
    accept the refusal — Decision 1 applied *cheaply*, since no coverage is lost. DAL/CHTR/BKR: ship
    the sign fix and carry the net-vs-gross residual as LD-8, which already states honestly that it
    has no mechanical detector. Precedent is this repo's own credit-curve episode
    (`prd-…-2026-08-03/.memlog.md:26-29`): 15-of-20 dark, resolved by fixing five real evidence bugs
    and *keeping* real evidence, never by fabricating a rung.
12. **T2.7 gains two acceptance criteria v3 could not state**: an *issuer-level* outcome assertion
    (blackout, not a quietly degraded rate — the year-level test cannot show this), and **COF's
    published-value delta being exactly $0.00**, which converts item 3 from an argument into a
    measurement.
13. **W2b must report DAL/CHTR/BKR's largest published-value swing** as a percentage of current
    published value. It is the one number that could reopen item 11 — and if it does, it reopens for
    *that name*, not for the ruling.
14. **T2.12 added** (Windows-only): plant one synthetic negative-interest row in the parity fixture,
    which today has **zero** (`(Select-String … '"interest":\s*-').Count` → `0`). It completes no
    comparison now; it means the discriminating case is already present when the Android export is
    regenerated at port time, so the *existing* instrument catches LD-10 on first run.
15. **LD-10 and LD-11 opened**, both with mechanical detectors. **Per-wave delta baselines** (R-4)
    stand: each wave measures its own baseline at wave start, the protected failing set is named and
    never counted.

**One argument deliberately withdrawn**

16. The orchestrator's case for LD-1's urgency leaned on *"shipping T2.5 without the bridge fix is
    strictly worse than not shipping."* With Android deferred, that half-shipped scenario does not
    exist and **the argument is void**. It is not recycled anywhere in v4. LD-1 stands on its
    Windows-only merits, which the brief already measured (`brief.md:30-32`).

## 10. Changelog, v2 to v3

Driven by the r3 round (Sensei `revise`, 12 P0 / 5 passes; Advisor `revise`, 2 P0 / 5 passes) and
by three facts established directly against the code by the orchestrator rather than delegated.

**Applied in this revision:**

1. **`fuse` read; Sensei P0-1 closed with the opposite of its concern.** No double division exists —
   `fuse` weights by `1.0/variance` alone and `observations` is a label. T3.8 was correct as
   specified. Pinned so it is not re-derived, with **K10** added for the zero-spread case.
2. **Pause trigger (a′)/(b′): an anchor going unavailable is an unconditional stop.** v2's carve-out
   for *"T2.7's declared refusal"* let an anchor go dark without forcing a stop. Confirmed against
   `edgar.rs:1099-1100` and `driver_resolution.rs:236-239`: channel refusal is issuer blackout.
   Both r3 reviewers reached this independently; it is the most consequential fix in v3.
3. **The affected set is pre-registered before the wave runs** — COF, DAL, CHTR, BKR; no anchor.
   Trigger (c) now fires on any issuer outside it.
4. **The fast-checks search was broken and is now executed, not asserted.** v2's `grep -nE` pattern
   returns **0 rows with all three `.abs()` sites intact**; the PowerShell replacement returns
   exactly `:551`, `:907`, `:1590`. Both were run before this sentence was written. The tool is
   named as PowerShell throughout.
5. **A second clause added to the verification rule**: an acceptance check must be executed against
   a known-positive case before it is written into the plan. This is the run's fourth instance of
   the same failure class and the first one the rule itself caused.
6. **`valuation_gap_attribution::` added to the mandatory gate** (Advisor P0-2). `derive_wacc` has
   two callers, not one, and the second reaches a *"CHTR-class ordering failures"* path with 15
   tests no other gate command runs.
7. **Pause trigger (e): the sites that *depended on* the abs.** Every test building an `FcfPoint`
   through `with_operating_drivers` had its inputs silently abs'd. `valuation_baseline.rs:900` is
   such a read site inside a file §2.0 declares untouchable — the deadlock is a stop-and-report,
   **never a test edit**.
8. **§6.2's "Wave 2's behaviour change is *proved* absent" is struck**, along with risk row R2's
   version of it. That sentence survived from the inert version of the wave and was the one line
   that would have let a reader merge Wave 2 without live QA.
9. **LD-8** opened: T2.7's sign rule is one-sided. It never fires for a net-**expense** filer, whose
   series is equally net and is fitted as though gross. The correct rule keys on basis, not sign,
   and is unimplementable until item 6. This one outlives the run and its numbers look plausible.
10. **LD-9** opened: `measured_sample_size` sums periods, analysts and inputs across variants.
11. **§6.4's "LD-1 through LD-4"** corrected — LD-1 is closed by Wave 2 and the register runs to
    LD-9.

**The r3 P1/P2 tail, also applied:**

12. **T5.12 promoted to a pre-flight (§4.1), out of Wave 5.** An ordering defect, not a preference:
    F1 carries the live-QA exemption for Waves 3 **and** 5, Wave 3 runs in R1, and T5.12 sat in R3.
    Two waves would have spent an exemption two rounds before the proof entitling them to it, with
    no path to unwind if it failed. The cucumber baseline and the protected-set confirmation moved
    with it.
13. **§6.4's LD list corrected twice.** LD-1 is closed, not deferred. **LD-4 cannot be deferred at
    all** — its own detector is the T4.5 gate, and T4.5 is in this run, so the NCI-basis question is
    forced open by scheduled work. What is out of scope is *changing* the equivalence class. T4.5
    resolves and records the basis; no wave alters it.
14. **`docs/roic-target-specification.md` added to §5.** It existed as a task and in Wave 4's own
    list but not in the consolidated table a builder reconciles against — which is how a binding
    artifact quietly does not ship.
15. **J7 added: enumerate the sites that *depended on* the abs.** Executed rather than asserted, and
    the result reshapes the wave — **45 call sites across 5 files**, not the single site v2
    discussed. `cross_platform_parity.rs` holds 9 of them and was named by no reviewer and by no
    prior revision; the parity suite is therefore *inside* Wave 2's blast radius, not a check on it.
    Forty-four of the forty-five are tests, so the work is fixture archaeology and the tempting
    shortcut — run the suite, fix what turns red — is explicitly forbidden.

**Nothing from r3 remains unapplied.** v3 is submitted to round 4 complete.

---

## 11. Changelog, v1 to v2
...
## 12. Changelog, v0 to v1 (retained for the record)
