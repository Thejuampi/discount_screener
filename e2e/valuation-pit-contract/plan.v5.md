...
| R2 | **Wave 2 moves published valuations. This is intended, not a risk to be disproved.** | High, and unmitigated by construction | v2 carried v1's inert-version answer here — that T2.6 *proves* no number moves. That proof died with Juan's Q1 ruling. The controls are: a pre-registered affected set (COF, DAL, CHTR, BKR — no anchor), sign-preservation tests for both filer classes, mandatory live QA with all five gate commands, and pause triggers (a′)/(c)/(e). **No wave may cite a proof of inertness for Wave 2.** |
...
| predicted | stop condition |
|---|---|
| guard **fires**: COF only, published delta **$0.00** | **any other issuer whose guard fires is a stop** |
| guard **never fires**, numbers unchanged: DAL, CHTR, BKR | **any non-zero published delta for these three is a stop** — the arithmetic says zero |
| guard never fires, numbers **change**: the net-income-operating set | **unmeasured until T2.0.** Measure it, or declare it empty with evidence |
...
convention. Add **one** synthetic row carrying a negative interest value.
...
- **(c)** the accounting cost-of-debt channel refuses for any issuer outside the pre-registered
  affected set — **COF, DAL, CHTR, BKR** — which would mean net-filing is more widespread than the
  measurement suggests. The set is the prediction; a miss is a finding, not a tolerance.
...
| --- | --- | --- |
| Shell library | `cargo test --lib` | **MEASURED 2026-08-04: 520 passing, 22 ignored, 3 failing.** The plan inherited **518** from an earlier session. See §4.2 — the difference is real, explained, and a blocker for Stage 4. |
...
**Pre-registered affected set, recorded before Wave 2 runs.** `InterestIncomeExpenseNet` is
measured to resolve for **COF (19 years), DAL (15), CHTR (12), BKR (11)** — none of them an anchor.
This is the plan's prediction, not its permission: if Wave 2 changes any issuer outside this set,
or changes an anchor at all, that is an unpredicted effect and (b′) or (b) fires. Recording the set
is what makes the surprise detectable; it is not a reason to skip checking.
...
## 11. Changelog, v0 to v1 (retained for the record)
