...
fn valuation_decision_policy_goldens_execute_on_windows() {
    ...
    let contract: Contract = serde_json::from_str(&std::fs::read_to_string(path).expect("read contract"))
...
fn difference_bps(left: i64, right: i64) -> Option<i64> {
    ...
    Some((((i128::from(left) - i128::from(right)).abs() * 20_000 + denominator / 2) / denominator) as i64)
}
...
fn shared_tipranks_forecast_goldens_execute_on_windows() {
    ...
    let contract: serde_json::Value = serde_json::from_str(&std::fs::read_to_string(path).expect("read TipRanks contract"))
    ...
    for case in contract["fixtures"].as_array().expect("fixtures") {
        ...
        let simple = observations.iter().map(|item| item["target"].as_i64().unwrap()).sum::<i64>() / observations.len() as i64;
        ...
        for observation in observations {
            if observation["ageSeconds"].as_i64().unwrap() <= 7_776_000 {
                ...
                let replace = newest.get(&identity).map(|prior| prior["ageSeconds"].as_i64().unwrap() > observation["ageSeconds"].as_i64().unwrap()).unwrap_or(true);
                if replace { newest.insert(identity, observation); }
            }
        }
        let weighted = if newest.len() >= 3 && newest.values().all(|item| item["weightMillis"].as_i64().unwrap() > 0) {
            let total_weight: i64 = newest.values().map(|item| item["weightMillis"].as_i64().unwrap()).sum();
            Some((newest.values().map(|item| item["target"].as_i64().unwrap() * item["weightMillis"].as_i64().unwrap()).sum::<i64>() + total_weight / 2) / total_weight)
        } else { None };
        let expected = &case["expected"];
        assert_eq!(Some(simple), expected["simple"].as_i64(), "{}", case["name"]);
        assert_eq!(weighted, expected["weighted"].as_i64(), "{}", case["name"]);
        assert_eq!(newest.len() as i64, expected["identities"].as_i64().unwrap(), "{}", case["name"]);
    }
}
