...
pub struct FcfPoint {
    ...
    /// inferred from the value's sign, which is a different axis. This is the
    /// per-year basis the accounting cost-of-debt channel in
    /// `driver_resolution.rs` drops years on (LD-8, closed by that rule where
    /// the basis reaches a published number).
    ///
...
impl FcfPoint {
    ...
    /// Set once the winning `interest_expense_dollars` concept for this year is
    /// known, so the accounting cost-of-debt fit can drop the year on basis
    /// rather than on sign. See the field's own doc comment for why this is
    /// `Option<bool>`, never a defaulted `bool`.
...
mod tests {
    ...
    /// T2.7 at the issuer level, rewritten under R-24 (`ORCHESTRATOR-RULINGS.md`).
    ///
    /// The retired version proved this by writing one raw, un-negated filed
    /// value into a single year and relying on its *sign* alone -- exactly the
    /// rule-(A) mechanism R-24.1 rules out ("the sign of the filed value is NOT
    /// part of the contract"). It also asserted an issuer-wide refusal from a
    /// single bad year, which (D) no longer produces: dropping one year of four
    /// still leaves three to fit, as
    /// `a_net_basis_year_is_dropped_and_the_issuer_still_fits_on_its_remaining_years`
    /// in `driver_resolution` now covers directly. This rewrite instead marks
    /// every year `interest_is_net_basis = Some(true)` -- what `edgar.rs` would
    /// record for four years that each won on `InterestIncomeExpenseNet` -- so
    /// the fittable set is empty for the reason R-24 actually names: basis, not
    /// sign. The year-level test in `driver_resolution` proves the branch
    /// drops; only this one proves what an *empty* fit costs at the full
    /// `compute()` level, because there is no lower rung to fall to -- no
    /// producer of a market yield or a rated spread exists anywhere in the
    /// tree, so the refusal reaches the caller as a terminal error rather than
    /// as a weaker number.
    #[test]
    fn a_net_basis_history_takes_the_fcff_path_dark_rather_than_degrading_the_rate() {
        ...
        assert!(
            ...
            "expected the FCFF path to go dark once every year of LIN's \
             InterestIncomeExpenseNet-basis history is dropped from the fit, got: {error}"
        );
    }
}
