...
pub fn analyze(c: &[HistoricalCandle], tf: &str) -> Option<FibAnalysis> {
    ...
    for (i, k) in w.iter().enumerate() {
        if k.high_cents > w[hi_i].high_cents { hi_i = i; }
        if k.low_cents < w[lo_i].low_cents { lo_i = i; }
    }
    ...
    let mut add = |ratio: f64, price: f64, kind: &'static str| {
        levels.push(FibLevel { ratio, price_cents: price.round() as i64, kind });
    };
    if up {
        add(0.0, hf, "anchor");
        for &r in &RETR { add(r, hf - r * range, "retracement"); }
        add(1.0, lf, "anchor");
        for &r in &EXT { add(r, hf + (r - 1.0) * range, "extension"); }
    } else {
        add(0.0, lf, "anchor");
        for &r in &RETR { add(r, lf + r * range, "retracement"); }
        add(1.0, hf, "anchor");
        for &r in &EXT { add(r, lf - (r - 1.0) * range, "extension"); }
    }
