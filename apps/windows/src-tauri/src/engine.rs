...
pub fn compute_chart_summary(candles: &[HistoricalCandle]) -> Option<ChartSummary> {
    ...
    let (macd_val, signal_val, histogram_val) = if closes.len() >= 26 {
        ...
        for (i, &c) in closes_f.iter().enumerate() {
            if i >= 12 { ema12 = c * k12 + ema12 * (1.0 - k12); }
            if i >= 26 {
        ...
        (Some(macd_now.round() as i64), Some(signal_now.round() as i64), Some(histogram_now.round() as i64))
    } else {
        (None, None, None)
    };
    ...
    let pos_52w = match (high_52w, low_52w) {
        (Some(h), Some(l)) if h > l => Some(((latest - l) as f64 / (h - l) as f64 * 100.0).clamp(0.0, 100.0)),
        _ => None,
    };
...
fn pivot_highs_idx(values: &[i64], lookback: usize) -> Vec<usize> {
    let mut out = Vec::new();
    if values.len() < lookback * 2 + 1 { return out; }
    for i in lookback..(values.len() - lookback) {
        let v = values[i];
        if (i - lookback..i).chain(i + 1..=i + lookback).all(|j| values[j] <= v) {
            out.push(i);
        }
    }
    out
...
pub fn detect_divergences(
    ...
    let mut out = Vec::new();
    let n = candles.len();
    if n < scan_window || n < period + lookback * 2 + 1 { return out; }
        ...
        if let (Some(r1), Some(r2)) = (r1, r2) {
            ...
            if p2 > p1 && r2 < r1 {
                ...
                out.push(Divergence {
                    ...
                    bias: "Bearish",
                    bars_ago, strength,
                    price_at_p1: p1, price_at_p2: p2, rsi_at_p1: r1, rsi_at_p2: r2,
                });
            } else if p2 < p1 && r2 > r1 {
...
fn compute_adx(candles: &[HistoricalCandle], period: usize) -> (Option<f64>, Option<f64>, Option<f64>) {
    if candles.len() < period * 2 + 1 { return (None, None, None); }
    let n = candles.len();
    ...
    for i in (period + 1)..n {
        ...
        if dx_den == 0.0 { continue; }
        let dx = 100.0 * (plus_di - minus_di).abs() / dx_den;
        adx_series.push((dx, plus_di, minus_di));
    }
    if adx_series.len() < period { return (None, None, None); }
    // ADX = SMA of DX for the first period, then Wilder-smoothed
    let mut adx = adx_series[..period].iter().map(|(dx, _, _)| dx).sum::<f64>() / period as f64;
...
fn compute_bollinger(
    ...
    if closes.len() < period { return (None, None, None, None, None); }
    let recent = &closes[closes.len() - period..];
    let mean = recent.iter().map(|&c| c as f64).sum::<f64>() / period as f64;
    let var  = recent.iter().map(|&c| (c as f64 - mean).powi(2)).sum::<f64>() / period as f64;
    let sd   = var.sqrt();
    let upper = mean + k * sd;
    let lower = mean - k * sd;
    let close = *closes.last().unwrap() as f64;
    let pct_b = if upper > lower { (close - lower) / (upper - lower) } else { 0.5 };
    let bw   = if mean > 0.0 { (upper - lower) / mean } else { 0.0 };
    (
...
fn compute_obv_slope(candles: &[HistoricalCandle], window: usize) -> Option<f64> {
    if candles.len() < window + 1 { return None; }
    let mut obv = 0.0f64;
    ...
    let denom = recent.iter().map(|v| v.abs()).fold(0.0f64, f64::max).max(1.0);
    Some(((last - first) / denom).clamp(-1.0, 1.0))
}

fn compute_volume_ratio(candles: &[HistoricalCandle], period: usize) -> Option<f64> {
    if candles.len() < period + 1 { return None; }
    let recent = &candles[candles.len() - period..];
    let mean = recent.iter().map(|c| c.volume as f64).sum::<f64>() / period as f64;
...
pub fn detect_candle_patterns(candles: &[HistoricalCandle]) -> Vec<DetectedPattern> {
    ...
    for offset in 0..scan {
        ...
        if body / range < 0.10 && range > 0.0 {
            out.push(DetectedPattern { name: "Doji".into(), bias: "Neutral", bars_ago: offset });
        }
        ...
        if body / range < 0.35 && lower_wick > 2.0 * body && upper_wick < body {
            out.push(DetectedPattern { name: "Hammer".into(), bias: "Bullish", bars_ago: offset });
        }
        ...
        if body / range < 0.35 && upper_wick > 2.0 * body && lower_wick < body {
            ...
            let name = if c.close_cents < c.open_cents { "Shooting Star" } else { "Inverted Hammer" };
            let bias = if name == "Shooting Star" { "Bearish" } else { "Bullish" };
            out.push(DetectedPattern { name: name.into(), bias, bars_ago: offset });
        }
        ...
        if idx >= 1 {
            ...
            if p_body_signed < 0 && c_body_signed > 0
                && c.close_cents > p.open_cents && c.open_cents < p.close_cents
                && c_body_signed.abs() > p_body
            {
                out.push(DetectedPattern { name: "Bullish Engulfing".into(), bias: "Bullish", bars_ago: offset });
            }
            // Bearish engulfing
            if p_body_signed > 0 && c_body_signed < 0
                && c.open_cents > p.close_cents && c.close_cents < p.open_cents
                && c_body_signed.abs() > p_body
            {
                out.push(DetectedPattern { name: "Bearish Engulfing".into(), bias: "Bearish", bars_ago: offset });
            }
        }
        ...
        if idx >= 2 {
            ...
            if p2_body_signed < 0 && star_small && c_body_signed > 0
                && c.close_cents > (p2.open_cents + p2.close_cents) / 2
            {
                out.push(DetectedPattern { name: "Morning Star".into(), bias: "Bullish", bars_ago: offset });
            }
            // Evening Star: bullish, star, strong bearish
            if p2_body_signed > 0 && star_small && c_body_signed < 0
                && c.close_cents < (p2.open_cents + p2.close_cents) / 2
            {
                out.push(DetectedPattern { name: "Evening Star".into(), bias: "Bearish", bars_ago: offset });
            }
        }
    }
    out
...
pub fn find_support_resistance(candles: &[HistoricalCandle], lookback: usize) -> SupportResistance {
    ...
    for i in lookback..(n - lookback) {
        ...
        let is_high = (i - lookback..i).chain(i + 1..=i + lookback)
            .all(|j| candles[j].high_cents <= h);
        let is_low  = (i - lookback..i).chain(i + 1..=i + lookback)
            .all(|j| candles[j].low_cents >= l);
        if is_high { pivots_high.push(h); }
        if is_low  { pivots_low.push(l); }
    }
    ...
    let cluster = |mut v: Vec<i64>| -> Vec<i64> {
        ...
        for x in v {
            if out.last().map(|&y| (x - y).abs() as f64 / y.max(1) as f64 > 0.005).unwrap_or(true) {
                out.push(x);
            }
        }
        out
...
fn classify_trend(c: &ChartSummary) -> &'static str {
    ...
    if strong_bull { "Bullish" }
    else if strong_bear { "Bearish" }
    else { "Neutral" }
}

fn classify_alignment(w: &'static str, d: &'static str, h: &'static str) -> &'static str {
    if w == "Bullish" && d == "Bullish" && h == "Bullish" { "BullStack" }
    else if w == "Bearish" && d == "Bearish" && h == "Bearish" { "BearStack" }
    else if w == "Unknown" || d == "Unknown" { "Unknown" }
    else { "Mixed" }
}
...
impl EvidenceAccumulator {
    ...
    pub fn add(&mut self, weight: f64, ramp: f64, label: &str) {
        ...
        let suffix = if clamped >= 0.5 { "++" } else if clamped > 0.0 { "+" } else if clamped >= -0.5 { "-" } else { "--" };
...
pub fn compute_sector_benchmarks(rows: &[CandidateRow]) -> HashMap<String, SectorBenchmarks> {
    ...
    for row in rows {
        ...
        if let Some(pe) = row.forward_pe_hundredths {
            if pe > 0 { pe_by_sector.entry(sector.clone()).or_default().push(pe); }
        }
...
pub fn score_fundamentals_v2(
    ...
    if fcf.is_none() && ocf.is_none() && roe_bps.is_none() && growth_bps.is_none()
        && de.is_none() && cash.is_none() && fwd_pe.is_none()
    {
        return (None, vec![]);
    }
    ...
    match (fcf, market_cap) {
        (Some(f), Some(mc)) if mc > 0 => {
            ...
            acc.add(V2_FUND_FCF_WEIGHT, smooth_ramp(yield_frac, V2_FUND_FCF_YIELD_LOWER, V2_FUND_FCF_YIELD_UPPER), "FCFy");
        }
        (Some(f), _) => {
        ...
        (None, _) => {
            if let Some(o) = ocf {
                acc.add(V2_FUND_OCF_FALLBACK_WEIGHT, if o > 0 { 1.0 } else { -1.0 }, "OCF");
            }
        }
    }
    ...
    if let Some(roe) = roe_bps {
        ...
        acc.add(V2_FUND_ROE_WEIGHT, smooth_ramp(roe as f64, lower, upper), label);
    }

    if let Some(g) = growth_bps {
        acc.add(V2_FUND_GROWTH_WEIGHT, smooth_ramp(g as f64, V2_FUND_GROWTH_LOWER_BPS, V2_FUND_GROWTH_UPPER_BPS), "Growth");
    }

    if let Some(d) = de {
        acc.add(V2_FUND_BALANCE_WEIGHT, -smooth_ramp(d as f64, V2_FUND_BALANCE_DE_LOW, V2_FUND_BALANCE_DE_HIGH), "D/E");
    } else if let (Some(c), Some(d)) = (cash, debt) {
        acc.add(V2_FUND_BALANCE_WEIGHT, if c >= d { 1.0 } else { -0.5 }, "Bal");
    }
    ...
    if let Some(pe) = fwd_pe {
        if pe > 0 {
            ...
            acc.add(V2_FUND_PE_WEIGHT, -smooth_ramp(pe as f64, lower, upper), label);
        }
    }
...
pub fn score_technicals_v3(
    ...
    let trend_score = {
        ...
        let frame_pts = |t: &str| match t {
            "Bullish" => 1.0, "Bearish" => -1.0, _ => 0.0,
        };
        if weekly.is_some() { acc += 35.0 * frame_pts(weekly_trend); weight += 35.0; }
        if daily.is_some()  { acc += 35.0 * frame_pts(daily_trend);  weight += 35.0; }
        if hourly.is_some() { acc += 15.0 * frame_pts(hourly_trend); weight += 15.0; }
        ...
        if weight > 0.0 { Some(((acc / weight) * 100.0).round() as i32) } else { None }
    };
    if let Some(s) = trend_score { signals.push(format!("Trend{}", if s > 0 { "+" } else if s < 0 { "-" } else { "·" })); }
    ...
    let momentum_score = {
        ...
        if let Some(d) = daily {
            ...
            if let Some(rsi) = d.rsi {
                ...
                let r = if rsi > 80.0 { r * 0.5 } else if rsi < 20.0 { r * 0.5 } else { r };
                acc += 30.0 * r; weight += 30.0;
                signals.push(format!("RSI{}", if rsi >= 60.0 { "+" } else if rsi <= 40.0 { "-" } else { "·" }));
            }
            if let (Some(macd), Some(sig)) = (d.macd_cents, d.signal_cents) {
                let dir = if macd > sig { 1.0 } else if macd < sig { -1.0 } else { 0.0 };
                acc += 25.0 * dir; weight += 25.0;
            }
            if let Some(h) = d.histogram_cents {
                if d.latest_close_cents > 0 {
                    let ratio = h as f64 / d.latest_close_cents as f64;
                    acc += 15.0 * smooth_ramp(ratio, -0.005, 0.005); weight += 15.0;
                }
            }
        }
    ...
    let volume_score = if let Some(d) = daily {
        ...
        if let Some(obv) = d.obv_slope {
            acc += 60.0 * obv; weight += 60.0;
            if obv > 0.3 { signals.push("OBV+".into()); } else if obv < -0.3 { signals.push("OBV-".into()); }
        }
        if let Some(vr) = d.volume_ratio {
            // Volume spike with up bar = bullish, with down bar = bearish
            let last_close = d.latest_close_cents;
            let prev_close = daily_candles.iter().rev().nth(1).map(|c| c.close_cents).unwrap_or(last_close);
            let dir = if last_close > prev_close { 1.0 } else if last_close < prev_close { -1.0 } else { 0.0 };
            // Ramp volume_ratio: 1.0 = neutral, 2.0+ = strong signal
            let intensity = ((vr - 1.0).max(0.0)).min(1.0);
            acc += 40.0 * intensity * dir; weight += 40.0;
        }
        if weight > 0.0 { Some(((acc / weight) * 100.0).round() as i32) } else { None }
    } else { None };
    ...
    let pattern_score = {
        ...
        for p in &patterns {
            let recency_w = if p.bars_ago == 0 { 1.0 }
                            else if p.bars_ago == 1 { 0.6 }
                            else { 0.3 };
            let bias_val = match p.bias { "Bullish" => 1.0, "Bearish" => -1.0, _ => 0.0 };
            acc += recency_w * bias_val;
            total_w += recency_w;
            if p.bars_ago <= 1 {
                let suffix = match p.bias { "Bullish" => "+", "Bearish" => "-", _ => "·" };
                signals.push(format!("{}{}", p.name.split_whitespace().next().unwrap_or("Pat"), suffix));
            }
        }
        ...
        for div in &divergences_preview {
            ...
            let kind_w = if div.kind.starts_with("Regular") { 1.5 } else { 1.0 };
        ...
        if total_w > 0.0 { Some(((acc / total_w) * 60.0).clamp(-100.0, 100.0).round() as i32) } else { None }
    };
    ...
    if alignment == "BullStack" { signals.push("Stack++".into()); }
    else if alignment == "BearStack" { signals.push("Stack--".into()); }
    ...
    let composite = {
        ...
        for (score, weight) in entries {
            if let Some(s) = score { acc += s as f64 * weight; w += weight; }
        }
        if w > 0.0 { Some((acc / w).round() as i32) } else { None }
    };

    let breakdown = TechnicalBreakdown {
        trend_score, momentum_score, volatility_score, volume_score, pattern_score,
        alignment, weekly_trend, daily_trend, hourly_trend,
        patterns, levels, divergences,
    };
...
pub fn score_technicals_v2(chart: &ChartSummary) -> (Option<i32>, Vec<String>) {
    ...
    if let Some(ema20) = chart.ema20_cents {
        if ema20 > 0 {
            ...
            acc.add(V2_TECH_TREND_PRICE_20_WEIGHT, smooth_ramp(delta, -V2_TECH_TREND_DELTA_BOUND, V2_TECH_TREND_DELTA_BOUND), "Px/20");
        }
        if let Some(ema50) = chart.ema50_cents {
            if ema50 > 0 {
                let delta = (ema20 - ema50) as f64 / ema50 as f64;
                acc.add(V2_TECH_TREND_20_50_WEIGHT, smooth_ramp(delta, -V2_TECH_TREND_DELTA_BOUND, V2_TECH_TREND_DELTA_BOUND), "20/50");
                if let Some(ema200) = chart.ema200_cents {
                    if ema200 > 0 {
                        let delta = (ema50 - ema200) as f64 / ema200 as f64;
                        acc.add(V2_TECH_TREND_50_200_WEIGHT, smooth_ramp(delta, -V2_TECH_TREND_DELTA_BOUND, V2_TECH_TREND_DELTA_BOUND), "50/200");
                    }
                }
            }
        }
    ...
    if let Some(hist) = chart.histogram_cents {
        ...
        acc.add(V2_TECH_HISTOGRAM_WEIGHT, smooth_ramp(ratio, -V2_TECH_HISTOGRAM_BOUND, V2_TECH_HISTOGRAM_BOUND), "Hist");
    }

    if let (Some(macd), Some(signal)) = (chart.macd_cents, chart.signal_cents) {
        let dir = if macd > signal { 1.0 } else if macd < signal { -1.0 } else { 0.0 };
...
pub fn score_forecast_v2(row: &CandidateRow) -> (Option<i32>, Vec<String>) {
    ...
    let target_fair = row.intrinsic_value_cents.checked_sub(0).filter(|&v| v > 0).map(|_| row.intrinsic_value_cents);
    let target_fair = if row.intrinsic_value_cents > 0 { Some(row.intrinsic_value_cents) } else { None };
    let analyst_count = row.analyst_opinion_count;
    let rec_count = {
        let trend_sum = [row.strong_buy_count, row.buy_count, row.hold_count, row.sell_count, row.strong_sell_count]
            .iter().filter_map(|&x| x).sum::<u32>();
    ...
    if let Some(fair) = target_fair {
        if analyst_count.map(|c| c >= V2_FORECAST_MIN_ANALYST_OPINIONS).unwrap_or(false) {
            ...
            if target_reliability > 0.0 {
                ...
                acc.add(weight, smooth_ramp(upside_bps, V2_FORECAST_UPSIDE_LOWER_BPS, V2_FORECAST_UPSIDE_UPPER_BPS), "Val");
    ...
    if let Some(rec) = row.recommendation_mean_hundredths {
        ...
        if rec_count.map(|c| c >= V2_FORECAST_MIN_ANALYST_OPINIONS).unwrap_or(false) && rec_reliability > 0.0 {
            let weight = V2_FORECAST_REC_WEIGHT * rec_reliability;
            acc.add(weight, -smooth_ramp(rec as f64, V2_FORECAST_REC_LOW_HUNDREDTHS, V2_FORECAST_REC_HIGH_HUNDREDTHS), "Rec");
            reliable_evidence_weight += weight;
        }
    }
    ...
    if let Some(count) = broadest {
        acc.add(V2_FORECAST_BREADTH_WEIGHT, analyst_breadth_ramp(count), "Cov");
        reliable_evidence_weight += V2_FORECAST_BREADTH_WEIGHT * analyst_coverage_reliability(Some(count));
    }

    // Uncertainty (spread between high and low targets)
    if let (Some(low), Some(high), Some(centre)) = (row.low_fair_value_cents, row.high_fair_value_cents, target_fair) {
        if centre > 0 && high > low {
            ...
            if target_reliability_no_freshness > 0.0 {
                ...
                acc.add(weight, -smooth_ramp(spread, 0.0, V2_FORECAST_UNCERTAINTY_BOUND), "Unc");
                reliable_evidence_weight += weight;
            }
        }
    }
    ...
    if target_fair.is_some() && analyst_count.map(|c| c >= V2_FORECAST_MIN_ANALYST_OPINIONS).unwrap_or(false) {
    ...
    if let Some(dcf) = row.dcf_value_cents {
        if dcf > 0 {
            let dcf_upside_bps = ((dcf - row.market_price_cents) as f64 / dcf as f64 * 10_000.0) as f64;
            let weight = 15.0;
            acc.add(weight, smooth_ramp(dcf_upside_bps, V2_FORECAST_UPSIDE_LOWER_BPS, V2_FORECAST_UPSIDE_UPPER_BPS), "DCF");
            reliable_evidence_weight += weight;
            has_valuation_anchor = true;
        }
    }
...
pub fn composite_score_v2(fund: Option<i32>, tech: Option<i32>, forecast: Option<i32>) -> i32 {
    let coverage = [fund, tech, forecast].iter().filter(|x| x.is_some()).count() as i32;
...
pub fn compute_setup_score(
    ...
    if let Some(b) = breakdown {
        ...
        if let Some(p) = b.pattern_score {
            if p > 40       { score += 5.0; }
            else if p < -40 { score -= 5.0; }
        }
        // Recent divergences add weighty signal
        for div in &b.divergences {
            if div.bars_ago <= 2 {
                let kind_w = if div.kind.starts_with("Regular") { 8.0 } else { 4.0 };
    ...
    // ── Gap to analyst target (stocks only) ──────────────────────────────────
    if !is_crypto_or_etf {
        if gap_bps <= 0       { score -= 20.0; }
        else if gap_bps >= 2000 { score += 5.0; }
    }
    ...
    if !is_crypto_or_etf {
        if let (Some(fcf), Some(mc)) = (fcf_dollars, market_cap_dollars) {
            if mc > 0 {
                ...
                if ratio < -0.10      { score -= 15.0; }
                else if ratio > 0.05  { score += 5.0; }
            }
        }
    }
    ...
    if buys >= 2 && buys >= sells       { score += 5.0; }
    else if sells >= 5 && buys == 0     { score -= 5.0; }

    let final_score = score.round().clamp(-100.0, 100.0) as i32;

    let label = if final_score >= 70       { "StrongBuy"   }
                else if final_score >= 50  { "Buy"         }
                else if final_score >= 30  { "Accumulate"  }
                else if final_score >= 10  { "Watch"       }
                else if final_score >= -10 { "Hold"        }
                else if final_score >= -40 { "Avoid"       }
                else                       { "StrongAvoid" };
...
pub fn decision_state(
    ...
    if confidence == ConfidenceBand::Low || gap_bps <= 0 || composite < DECISION_AVOID_THRESHOLD {
        return "Avoid";
    }
...
impl ScreenerState {
    ...
    pub fn ingest_signal(&mut self, signal: ExternalValuationSignal) {
        self.signals.insert(signal.symbol.clone(), signal);
    }

    pub fn ingest_fundamentals(&mut self, fund: FundamentalSnapshot) {
    ...
    pub fn candidate_rows(&self) -> Vec<CandidateRow> {
        let mut rows: Vec<CandidateRow> = self
            ...
            .map(|snap| {
                ...
                let ss = external_signal_status(signal, snap.market_price_cents, self.signal_max_age_seconds);
                ...
                CandidateRow {
                    ...
                    intrinsic_value_cents: snap.intrinsic_value_cents,
                    gap_bps: g,
                    qualification: qual,
                    confidence: conf,
                    signal_status: ss,
    ...
    pub fn detail(&self, symbol: &str) -> Option<SymbolDetail> {
        ...
        Some(SymbolDetail {
            ...
            next_earnings_epoch: snap.next_earnings_epoch,
            chart_patterns: self.daily_candles.get(symbol)
                .map(|c| crate::chart_patterns::detect(c, 3, "1D"))
                .unwrap_or_default(),
            fib: self.daily_candles.get(symbol)
...
pub fn gap_bps(market_price_cents: i64, intrinsic_value_cents: i64) -> i32 {
    if intrinsic_value_cents <= 0 {
        return i32::MIN;
    }
    let gap = (intrinsic_value_cents - market_price_cents) as f64
        / intrinsic_value_cents as f64
        * 10_000.0;
    gap.clamp(i32::MIN as f64, i32::MAX as f64) as i32
}

pub fn qualification(profitable: bool, gap: i32, min_gap_bps: i32) -> QualificationStatus {
    if !profitable {
        QualificationStatus::Unprofitable
    } else if gap >= min_gap_bps {
