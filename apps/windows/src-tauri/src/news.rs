...
const POSITIVE: &[&str] = &[
    "beats", "beat", "exceeds", "exceeded", "surges", "surge", "surged",
    "rally", "rallies", "rallied", "soars", "soared", "jumps", "jumped",
    "climbs", "climbed", "upgrade", "upgrades", "upgraded", "outperform",
    "raises", "raised", "boost", "boosted", "boosts", "strong", "stronger",
    "record", "all-time high", "breakthrough", "approves", "approved",
    "approval", "wins", "won", "winning", "partnership", "deal", "acquisition",
    "expansion", "expand", "growth", "growing", "innovative", "milestone",
    "positive", "bullish", "buy", "rated buy", "overweight", "favorable",
    "exceeds estimates", "beat estimates", "tops estimates", "strong demand",
    "guidance raised", "raises guidance", "raises forecast", "blowout",
    "stellar", "robust", "momentum", "tailwind", "tailwinds",
];

const NEGATIVE: &[&str] = &[
    "misses", "missed", "miss", "falls", "fall", "fell", "drops", "drop",
    "dropped", "plunges", "plunged", "tumbles", "tumbled", "slides", "slid",
    "downgrade", "downgrades", "downgraded", "cuts", "cut", "warning",
    "warns", "warned", "lawsuit", "investigation", "investigated", "fraud",
    "concerns", "concern", "worry", "worried", "fears", "fear", "bearish",
    "selloff", "sell-off", "crash", "loss", "losses", "decline", "declined",
    "declines", "underperform", "sell", "rated sell", "underweight",
    "risk", "risky", "risks", "uncertainty", "uncertain", "delay", "delayed",
    "delays", "weak", "weakness", "weaker", "disappoints", "disappointed",
    "disappointment", "miss estimates", "below estimates", "lowered guidance",
    "cuts guidance", "guidance cut", "slashes", "slumps", "slump", "headwind",
    "headwinds", "recession", "bankruptcy", "default", "downturn",
    "subpoena", "fine", "fined", "penalty", "settlement", "recall",
];
...
impl NewsCache {
    ...
    pub fn get(&self, symbol: &str) -> Option<NewsBundle> {
        ...
        if at.elapsed().as_secs() > CACHE_TTL_SECS { return None; }
        Some(b.clone())
    }
...
pub fn fetch_news(client: &Client, symbol: &str) -> Result<NewsBundle, String> {
    ...
    let xml = client.get(&url).send()
        .map_err(|e| format!("news fetch {}: {}", symbol, e))?
        .text()
        .map_err(|e| format!("news body {}: {}", symbol, e))?;

    let items = parse_rss(&xml);
    let scored: Vec<NewsItem> = items.into_iter().map(|mut it| {
        it.sentiment_score = score_headline(&it.title);
        it
    }).collect();

    let mut pos = 0u32; let mut neg = 0u32; let mut neu = 0u32;
    for it in &scored {
        if it.sentiment_score > 5 { pos += 1; }
        else if it.sentiment_score < -5 { neg += 1; }
        else { neu += 1; }
    }
    let aggregate = if scored.is_empty() { 0 } else {
...
fn score_headline(title: &str) -> i32 {
    ...
    let mut neg = 0i32;
    for kw in POSITIVE {
        if lower.contains(kw) { pos += 1; }
    }
    for kw in NEGATIVE {
        if lower.contains(kw) { neg += 1; }
    }
    let raw = pos - neg;
