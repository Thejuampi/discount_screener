...
test("strong adverse technical demotes long ActNow to Avoid", () => {
  const plan = buildConditionalPlan(
    baseRow({
      technical_score: -50,
      price_path: {
  ...
  assert.equal(plan.stance, "Avoid");
  assert.equal(isActionablePriority(plan), false);
  assert.ok(plan.caution.some((c) => c.textKey.includes("tech")));
});

test("mild adverse technical demotes long ActNow to WaitZone", () => {
  const plan = buildConditionalPlan(
    baseRow({
      technical_score: -20,
      price_path: {
...
test("strong bullish technical demotes short ActNow to Avoid", () => {
  assert.equal(applyTechnicalConsistency("ActNow", 50, "short"), "Avoid");
  assert.equal(applyTechnicalConsistency("ActNow", 20, "short"), "WaitZone");
  assert.equal(applyTechnicalConsistency("ActNow", -10, "short"), "ActNow");
  assert.equal(applyTechnicalConsistency("ActNow", 20, "long"), "ActNow");
});

test("mild positive technical does not block long ActNow", () => {
  const plan = buildConditionalPlan(
    baseRow({
      technical_score: 19,
      price_path: {
