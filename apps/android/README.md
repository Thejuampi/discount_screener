...
- **Earnings tab** (Android-only): pre-report gate on implied-move vs the ticker's own median |AR|, cheapness vs DCF, hedge cap. SUE slope from Alpha Vantage 16–20 quarters is a diagnostic on the card. Own last-four revenue trail can cut a Hold to half size. Knobs: `shared/contracts/earnings-gate-policy.yaml`. PRD: `_bmad-output/planning-artifacts/prd-pre-earnings-risk-gate-2026-08-27.md`.
