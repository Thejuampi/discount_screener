...
- **Discovery tab** (manual): broad US equity universe separate from startup profiles. Guided UX: **Create US list** → **Score list** → filter by min score / model. **Update list** prefers the live [NASDAQ Trader symbol directory](https://www.nasdaqtrader.com/dynamic/SymDir/nasdaqlisted.txt) (~7k non-ETF equities; falls back to `assets/universes/us_total.txt`) and merges membership in SQLite (add/remove/keep; no quote scrape). **Score list** fetches only the minimum quote + 1Y chart data needed for `OpportunityEngine`, shows determinate progress, keeps partial results, and never changes membership. Confirm dialogs guard list update, long scoring, and clear. Startup only **reads** Discovery state from the DB — it never auto-recreates or auto-refreshes. Min score and scoring model are persisted in SQLite meta. Offline asset refresh: `pwsh ./scripts/refresh-us-total-universe.ps1`.
