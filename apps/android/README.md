...
- **Earnings tab** (Android-only). PRD: `_bmad-output/planning-artifacts/prd-pre-earnings-risk-gate-2026-08-27.md`. Spec: `_bmad-output/specs/spec-earnings-gate-identities/SPEC.md`. Policy `/2`: `shared/contracts/earnings-gate-policy.yaml`. Import book (Chase/J.P. Morgan) lives on Earnings, System, and Positions. Positions is the book home. Contract: `shared/contracts/advisor-csv-import-v1.yaml` (`advisor-csv-import/3`).
...
- [Android Positions](../../docs/android-positions-ux.md) shows stock exposure, research labels, and local position facts. The dashboard header hides on downward content scroll and returns on upward scroll. Populated Positions keeps Import book in its menu.
