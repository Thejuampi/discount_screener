...
Press `h` to toggle from `Snapshot` to `History`. The History tab provides a graph-first view of time-series data stored in the SQLite database.

Controls inside History:

- `h`: toggle back to `Snapshot`
- `g`: toggle between `Graphs` and `Table` subviews
- `1` through `5`: switch metric groups (`Core`, `Fundamentals`, `Relative`, `DCF`, `Chart`)
- `[` / `]`: move backward or forward through time windows
- `j` / `k`: scroll rows
- `n` / `p`: next or previous symbol in the active ranked set
- `e`: export full revision history as CSV
- Navigation keys `Home`, `End`, `PageUp`, `PageDown` follow the same model as the main table

For metric group details, time window descriptions, CSV export file layout, and direct SQL queries, see [HISTORY_TIME_SERIES.md](HISTORY_TIME_SERIES.md).

Representative current detail header:

```text
TICKER DETAIL  |  j/k next ticker  |  1-6 range  |  [/] cycle  |  ←/→ replay  |  h history  |  w watch  |  l logs  |  Backspace or d or Enter close  |  q quit  |  Ctrl+C quit
```
